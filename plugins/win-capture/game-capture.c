...
#define SETTING_IGNORE_RENDER_NOTINFULLSCREEN "ignore_render_notinfullscreen"
...
#define TEXT_IGNORE_RENDER_NOTINFULLSCREEN   obs_module_text("GameCapture.IgnoreRenderNotInFullScreen")
...
struct game_capture_config {
		...
		bool                          ignore_render_leave_fullscreen;
		uint32_t                      scale_cx;
		uint32_t                      scale_cy;
		bool                          cursor;
		bool                          force_shmem;
...
static bool isfullwindow(struct game_capture *gc)
{
		...
		HWND window = gc->window;

		if (!window) {
				return false;
		}
		if (window != cur_window) return false;
		...
		obs_data_t *settings, const char *window)
{
		...
		cfg->ignore_render_leave_fullscreen = obs_data_get_bool(settings,
				SETTING_IGNORE_RENDER_NOTINFULLSCREEN);
...
static void game_capture_render(void *data, gs_effect_t *effect)
{
		...
		if (gc->config.mode == CAPTURE_MODE_ANY && gc->config.ignore_render_leave_fullscreen) {
				if (!isfullwindow(gc)) return;
		}
...
static void game_capture_defaults(obs_data_t *settings)
{
		...
		obs_data_set_default_bool(settings, SETTING_IGNORE_RENDER_NOTINFULLSCREEN, false);
		...
		obs_property_t *p, obs_data_t *settings)
{
		...
		p = obs_properties_get(ppts, SETTING_IGNORE_RENDER_NOTINFULLSCREEN);
...
static obs_properties_t *game_capture_properties(void *data)
{
		...
		* values */
		...
		obs_properties_add_bool(ppts, SETTING_IGNORE_RENDER_NOTINFULLSCREEN,
				TEXT_IGNORE_RENDER_NOTINFULLSCREEN);
