...
struct ffmpeg_data {
	...
	//audio_tracks is a bitmask storing the indexes of the mixes selected.
	int                audio_tracks;
		...
		struct ffmpeg_cfg *config)
{
	...
	memset(data, 0, sizeof(struct ffmpeg_data));
	data->config = *config;
	data->num_audio_streams = config->number_audio_mixes;
 ...
 */
int getTrackOrder(int track_config, int mix_index)
{
	int position = 0;
	for (int i = 0; i < mix_index; i++) {
		if (track_config & 1 << i)
			position++;
	}
	return position;
...
static void receive_audio(void *param, int mix_idx, struct audio_data *frame)
{
	struct ffmpeg_output *output = param;
	struct ffmpeg_data   *data   = &output->ff_data;
	...
	if (!(data->audio_tracks & (1 << mix_idx )))
		return;

	// get track order (first selected, etc ...)
	track_order = getTrackOrder(data->audio_tracks, mix_idx);
...
static bool try_connect(struct ffmpeg_output *output)
{
	...
	config.number_audio_mixes = (int)obs_data_get_int(settings, "number_mixes");
	config.audio_tracks = (int)obs_data_get_int(settings, "audio_tracks");
