...
private:
	...
	void get_output_tex(ComPtr<ID3D11Texture2D> &output_tex, ID3D11Texture2D *from);
...
{
	*amf_gpu_obj = new amf_windows_impl();
	return AMF_OK;
}
//------------------------------------------------------------------------------------------------------
AMF_RESULT amf_windows_impl::init(amf::AMFContext *context, uint32_t adapter)
{
	AMFOBS_RETURN_IF_FALSE(context != nullptr, AMF_INVALID_ARG, "invalid input parameter");
	AMF_RESULT res;
	res = init_d3d11(adapter);
	AMFOBS_RETURN_IF_FAILED(res, "init_d3d11 failed with res = %d", res);

	res = context->InitDX11(dx_device, AMF_DX11_1);
	AMFOBS_RETURN_IF_FAILED(res, "AMFContext::InitDX11() failed with res = %d", res);
...
AMF_RESULT amf_windows_impl::init_d3d11(uint32_t adapterIndex)
{
	...
	AMFOBS_RETURN_IF_FALSE(dxgi != nullptr || d3d11 != nullptr, AMF_FAIL,
			       "Couldn't get D3D11/DXGI libraries? That definitely shouldn't be possible.");
	...
	AMFOBS_RETURN_IF_HR_FAILED(hr, AMF_FAIL, "CreateDXGIFactory1 failed: 0x%lX", hr);

	hr = factory->EnumAdapters(adapterIndex, &adapter);
	AMFOBS_RETURN_IF_HR_FAILED(hr, AMF_FAIL, "EnumAdapters failed: 0x%lX", hr);

	adapter->GetDesc(&desc);

	AMFOBS_RETURN_IF_FALSE(desc.VendorId == AMD_VENDOR_ID, AMF_FAIL,
			       "Seems somehow AMF is trying to initialize on a non-AMD adapter");

	hr = create_device(adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0, nullptr, 0, D3D11_SDK_VERSION, &device,
			   nullptr, &context);
	AMFOBS_RETURN_IF_HR_FAILED(hr, AMF_FAIL, "D3D11CreateDevice failed: 0x%lX", hr);
...
AMF_RESULT amf_windows_impl::add_output_tex(ComPtr<ID3D11Texture2D> &output_tex, ID3D11Texture2D *from)
{
	...
	AMFOBS_RETURN_IF_HR_FAILED(hr, AMF_FAIL, "CreateTexture2D failed to create texture: 0x%lX", hr);
	return AMF_OK;
}
//------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------
void amf_windows_impl::get_output_tex(ComPtr<ID3D11Texture2D> &output_tex, ID3D11Texture2D *from)
{
	if (!get_available_tex(output_tex))
		add_output_tex(output_tex, from);
}
//------------------------------------------------------------------------------------------------------
...
{
	AMFOBS_RETURN_IF_FALSE(amf_context != nullptr, AMF_NOT_INITIALIZED, "Not initialized");
	...
	if (handle == GS_INVALID_HANDLE) {
		*next_key = lock_key;
		AMFOBS_RETURN_IF_FALSE(handle != GS_INVALID_HANDLE, AMF_FAIL, "Encode failed: bad texture handle");
	}

	get_tex_from_handle(handle, &km, &input_tex);
	AMFOBS_RETURN_IF_FALSE(input_tex != nullptr, AMF_FAIL, "get_tex_from_handle() failed");

	get_output_tex(output_tex, input_tex);
	AMFOBS_RETURN_IF_FALSE(output_tex != nullptr, AMF_FAIL, "get_output_tex() failed");
	...
	AMFOBS_RETURN_IF_FAILED(res, "AMFContext::CreateSurfaceFromDX11Native() failed with res = %d", res);
	return AMF_OK;
}
//------------------------------------------------------------------------------------------------------
