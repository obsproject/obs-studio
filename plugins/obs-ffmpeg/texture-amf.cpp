...
static bool amf_avc_init(void *data, obs_data_t *settings)
{
	...
	info("settings:\n"
	     ...
	     "\tpre-analysis: %s\n"
	     "\tparams:       %s",
	     rc_str, bitrate, qp, gop_size, preset, profile, bf, enc->cx, enc->cy, pa_enabled ? "true" : "false",
	     ffmpeg_opts);
...
static void amf_hevc_update_data(amf_base *enc, int rc, int64_t bitrate, int64_t qp)
{
	if (rc != AMF_VIDEO_ENCODER_HEVC_RATE_CONTROL_METHOD_CONSTANT_QP &&
		...
		if (rc == AMF_VIDEO_ENCODER_HEVC_RATE_CONTROL_METHOD_PEAK_CONSTRAINED_VBR ||
...
static bool amf_hevc_init(void *data, obs_data_t *settings)
{
	...
	info("settings:\n"
	     ...
	     "\tpre-analysis: %s\n"
	     "\tparams:       %s",
	     rc_str, bitrate, qp, gop_size, preset, profile, enc->cx, enc->cy, pa_enabled ? "true" : "false",
	     ffmpeg_opts);
...
static bool amf_av1_init(void *data, obs_data_t *settings)
{
	...
	info("settings:\n"
	     ...
	     "\tpre-analysis:         %s\n"
	     "\tparams:               %s",
	     rc_str, bitrate, qp, gop_size, preset, profile, bf, enc->cx, enc->cy, screen_content_tools_enabled ? "true" :
		 "false", palette_mode_enabled ? "true" : "false", pa_enabled ? "true" : "false", ffmpeg_opts);
