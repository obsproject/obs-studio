...
static int libsrt_open(URLContext *h, const char *uri)
{
	...
	/* In caller mode, srt_connect won't fail even if there is no established connection in non-blocking mode;
	 * so, we explicitly check for the socket status before okeying OBS_OUTPUT_SUCCESS .
	 */
