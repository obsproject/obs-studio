...
public:
	...
	string device_name;
	std::atomic<bool> enableChannelRouting;
...
WASAPISource::UpdateParams WASAPISource::BuildUpdateParams(obs_data_t *settings)
{
	...
	if (sourceType == SourceType::Input) {
		params.enableChannelRouting = obs_data_get_bool(settings, "enable_routing");
		bool routingUpdated = params.enableChannelRouting !=
...
void WASAPISource::UpdateSettings(UpdateParams &&params)
{
	...
	if (sourceType == SourceType::Input) {
		enableChannelRouting.store(std::move(params.enableChannelRouting), std::memory_order_relaxed);
		bool changed = false;
		for (int i = 0; i < MAX_AUDIO_CHANNELS; i++) {
...
void WASAPISource::Initialize()
{
	...
	size_t size = dst_speakers == SPEAKERS_UNKNOWN ? 1 : get_audio_size(format, dst_speakers, 1);
	audioCapacity = 1;
...
bool WASAPISource::ProcessCaptureData()
{
	...
	while (true) {
		...
		if (sourceType == SourceType::Input) {
			...
			if (enableChannelRouting.load(std::memory_order_relaxed) && resampler) {
...
static void GetWASAPIDefaultsInput(obs_data_t *settings)
{
	...
	obs_data_set_default_bool(settings, "enable_routing", false);
...
static bool channel_update_cb(void *vptr, obs_properties_t *props, obs_property_t *chanlist, obs_data_t *settings)
{
	...
	if (strcmp(device_id, "default") == 0) {
		device_name = GetWASAPIDefaultDeviceName(true);
	} else {
	...
	int inputChannels = GetWASAPIDeviceInputChannels(device_id);
	obs_property_list_clear(chanlist);
	obs_property_list_add_int(chanlist, obs_module_text("None"), -1);
	for (int j = 0; j < inputChannels; ++j) {
		char label[256];
		snprintf(label, sizeof(label), "%s channel %d", device_name.c_str(), j + 1);
			  ...
			  bool update_settings)
{
	...
	bool enable_routing = obs_data_get_bool(settings, "enable_routing");
	...
	for (int i = 0; i < MAX_AUDIO_CHANNELS; ++i) {
		...
		obs_property_set_visible(mix_channels[i], i < obs_channels && enable_routing);
		if (update_settings) {
			obs_data_set_int(settings, channel_str.c_str(), -1);
		}
...
static bool wasapi_enable_routing_changed(void *vptr, obs_properties_t *props, obs_property_t *devlist,
					  obs_data_t *settings)
{
	WASAPISource *data = (WASAPISource *)vptr;
	if (data == NULL) {
	...
	bool enable_routing = obs_data_get_bool(settings, "enable_routing");
	return reload_channel_props(data, props, devlist, settings,
				    data->enableChannelRouting.load(std::memory_order_relaxed) != enable_routing);
}
...
static obs_properties_t *GetWASAPIPropertiesInput(void *vptr)
{
	...
	if (data != nullptr) {
		property = obs_properties_add_bool(props, "enable_routing", obs_module_text("Routing"));
		obs_property_set_modified_callback2(property, wasapi_enable_routing_changed, data);
		obs_property_set_long_description(property, obs_module_text("Routing.Hint"));
		...
		for (int i = 0; i < MAX_AUDIO_CHANNELS; ++i) {
			...
			obs_property_set_visible(property, i < obs_channels && data->enableChannelRouting.load(
