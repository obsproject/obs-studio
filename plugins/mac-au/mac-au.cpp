...
void au_save_state(void *data, obs_data_t *settings)
{
	...
	CFDataRef state = au_plugin_save_state(ad->plugin.get());
	if (!state)
		return;
...
static bool create_new_au(au_data *ad, const char *uid)
{
	...
	// Pass new plugin params
	{
		ad->plugin = std::shared_ptr<au_plugin>(plugin, AuPluginDeleter{});
		...
		ad->sc_channels = ad->plugin->sidechain_num_channels;
		ad->has_sidechain.store((ad->sc_channels == 1 || ad->sc_channels == 2), std::memory_order_relaxed);
		if (!ad->has_sidechain.load(std::memory_order_relaxed))
			ad->sc_channels = 0;

		ad->has_view.store(ad->plugin->has_view, std::memory_order_relaxed);
	}
...
static void destroy_current_au(au_data *ad, obs_data_t *settings)
{
	...
	ad->bypass.store(true, std::memory_order_relaxed);
	if (ad->plugin) {
		au_plugin_hide_editor(ad->plugin.get());
		ad->plugin.reset();
	}
	ad->uid[0] = 0;
	ad->name[0] = 0;
...
static void au_load_state(au_data *ad, obs_data_t *settings)
{
	...
	if (hex && *hex) {
		...
		if (d) {
			au_plugin_load_state(ad->plugin.get(), d);
			CFRelease(d);
		}
	}
}
...
static void au_update(void *data, obs_data_t *settings)
{
	...
	if (!ad->plugin || !ad->has_sidechain.load(std::memory_order_relaxed))
		return;
...
static void *au_create(obs_data_t *settings, obs_source_t *filter)
{
	...
	ad->sc_copy_buffers[0] = (float *)bmalloc(FRAME_SIZE * 2 * sizeof(float));
...
static inline void preprocess_input(struct au_data *ad)
{
	...
	if (has_sc && sc_enabled) {
		...
		if (needs_resampling && ad->sc_resampler) {
			uint8_t *resampled[2] = {nullptr, nullptr};
			uint32_t out_frames = 0;
			uint64_t ts_offset = 0;

			if (audio_resampler_resample(ad->sc_resampler.get(), resampled, &out_frames, &ts_offset,
...
static inline void process(struct au_data *ad, const std::shared_ptr<au_plugin> &plugin)
{
	...
	if (!plugin->num_enabled_out_audio_buses)
		return;

	// Fill copy_buffers / sc_copy_buffers for this block
	preprocess_input(ad);
...
static struct obs_audio_data *au_filter_audio(void *data, struct obs_audio_data *audio)
{
	...
	if (!p->num_enabled_out_audio_buses)
		return audio;
