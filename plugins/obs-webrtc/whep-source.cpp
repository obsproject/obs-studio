...
void WHEPSource::OnFrameAudio(rtc::binary &msg,
...
void WHEPSource::StartThread()
{
	...
	do {
		...
		status = send_offer(bearer_token, endpoint_url, peer_connection,
	...
	do_log(LOG_DEBUG, "WHEP connected. Resource URL is: %s",
	       resource_url.c_str());
