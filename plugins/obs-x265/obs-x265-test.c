...
static void test_obs_x265_parse_options()
{
	struct obs_x265_options options;

	options = obs_x265_parse_options(NULL);
	CHECK(options.count == 0);
	CHECK(options.ignored_word_count == 0);
	obs_x265_free_options(options);

	options = obs_x265_parse_options("");
	CHECK(options.count == 0);
	CHECK(options.ignored_word_count == 0);
	obs_x265_free_options(options);

	options = obs_x265_parse_options("ref=3");
	...
	options = obs_x265_parse_options("ref=3 bframes=8");
