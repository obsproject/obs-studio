...
#ifdef OAUTH_ENABLED
constexpr const char *DATA_FILENAME = "obs-restream.json";
#endif
...
#ifdef OAUTH_ENABLED
void RestreamService::OBSEvent(obs_frontend_event event, void *priv)
{
	RestreamService *self = reinterpret_cast<RestreamService *>(priv);
...
void RestreamService::SaveOAuthsData()
{
	...
	if (!obs_data_save_json_pretty_safe(saveData, dataPath.c_str(), "tmp",
					    "bak"))
		blog(LOG_ERROR,
		     "[obs-restream][%s] Failed to save integrations data",
		     __FUNCTION__);
};

RestreamApi::ServiceOAuth *RestreamService::GetOAuth(const std::string &uuid,
						     obs_service_t *service)
{
	if (data == nullptr) {
		...
		if (!data) {
			blog(LOG_DEBUG,
			     "[obs-restream][%s] Failed to open integrations data: %s",
	...
	if (oauths.count(uuid) == 0) {
		...
		oauths.emplace(uuid,
			       std::make_shared<RestreamApi::ServiceOAuth>());
...
void RestreamService::ReleaseOAuth(const std::string &uuid,
				   obs_service_t *service)
{
