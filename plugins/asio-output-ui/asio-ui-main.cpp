/******************************************************************************
    ...
    This file is part of asio-output-ui which requires win-asio.
...
namespace {
constexpr int kMaxASIODeviceChannels = 32;
constexpr int kShiftWindow = 100;
// We use a global context for ASIO output because we allow a single output device.
...
void save_default_settings(obs_data_t *settings)
{
	...
	for (int i = 0; i < kMaxASIODeviceChannels; i++) {
		char key[32];
...
bool output_stop_and_wait()
{
	...
	output_running = obs_output_active(context.output);
...
void callback()
{
	...
	if (obsSettingsDialog) {
		...
		QPoint newPos(settingsRect.right() + kShiftWindow, settingsRect.top());
	...
	if (event == OBS_FRONTEND_EVENT_FINISHED_LOADING) {
		...
		if (context.output && obs_output_active(context.output)) {
			if (!output_stop_and_wait()) {
				blog(LOG_ERROR, "[asio_output_ui] Timed out while "
						"stopping output during shutdown");
			}
		} else {
			output_running = false;
		}
...
void obs_module_post_load(void)
{
	...
	if (output != nullptr) {
		...
		// so that users know that ASIO exists.
		addOutputUI();
	}
}
