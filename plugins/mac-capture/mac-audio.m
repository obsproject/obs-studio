...
static bool find_device_id_by_uid(struct coreaudio_data *ca)
{
    ...
    if (ca->default_device) {
        ...
        stat = AudioObjectGetPropertyData(kAudioObjectSystemObject, &addr, qual_size, &qual, &size, &ca->device_id);
        success = (stat == noErr);
    } else {
...
static char **coreaudio_get_channel_names(struct coreaudio_data *ca)
{
    ...
    for (uint32_t i = 0; i < ca->available_channels; i++) {
        ...
        struct dstr name;
        dstr_init(&name);
        if (ca_success(stat, ca, "coreaudio_init_format", "get channel names") && CFStringGetLength(cf_chan_name)) {
...
static bool coreaudio_init_format(struct coreaudio_data *ca)
{
    ...
    stat = get_property(ca->unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 1, &inputDescription, &size);
    ...
    if (ca->enable_downmix) {
        ...
        blog(LOG_INFO, "Channel map enabled: [%s] (%d channels available)", cm_str.array, ca->available_channels);
        dstr_free(&cm_str);

        stat = set_property(ca->unit, kAudioOutputUnitProperty_ChannelMap, SCOPE_OUTPUT, BUS_INPUT, ca->channel_map,
...
static bool coreaudio_init_buffer(struct coreaudio_data *ca)
{
    ...
    err = AudioUnitGetProperty(ca->unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &streamDescription,
                               &propertySize);
...
static void coreaudio_set_channels(struct coreaudio_data *ca, obs_data_t *settings)
{
    ...
    for (uint8_t i = 0; i < MAX_AUDIO_CHANNELS; i++) {
        ...
        int64_t found = obs_data_has_user_value(settings, setting_name) ? obs_data_get_int(settings, setting_name)
                                                                        : -1L;
                                       ...
                                       const char *device_config_name, uint32_t out_chan)
{
    ...
    if (prop) {
        ...
        obs_property_t *input_combo =
            obs_properties_add_list(props, name.array, label.array, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
        dstr_free(&label);
...
static obs_properties_t *coreaudio_properties(bool input, void *data)
{
    ...
    property = obs_properties_add_list(props, "device_id", TEXT_DEVICE, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
