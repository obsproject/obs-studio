...
static void *amf_hevc_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	// Picture Control
	int64_t keyinterv = obs_data_get_int(settings, KEYFRAME_INTERVAL);
	int64_t idrperiod =
		static_cast<int64_t>(keyinterv * enc->frame_rate.num);
...
void amf_hevc_defaults(obs_data_t *settings)
{
	...
	obs_data_set_default_bool(settings, PREPASSMODE, false);
