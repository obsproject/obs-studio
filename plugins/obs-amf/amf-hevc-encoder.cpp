...
static bool amf_hevc_update(void *data, obs_data_t *settings)
{
	...
	AMF_VIDEO_ENCODER_HEVC_RATE_CONTROL_METHOD_ENUM rcm =
		static_cast<AMF_VIDEO_ENCODER_HEVC_RATE_CONTROL_METHOD_ENUM>(
			obs_data_get_int(settings, P_RATECONTROLMETHOD));

	// Rate Control Properties
	int64_t bitrate = obs_data_get_int(settings, P_BITRATE) * 1000;
	if (rcm != AMF_VIDEO_ENCODER_HEVC_RATE_CONTROL_METHOD_CONSTANT_QP) {
...
static void *amf_hevc_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	if (result != AMF_OK)
		goto fail;
	enc->codec = amf_data::CODEC_ID_HEVC;
	...
	SET_AMF_VALUE_OR_FAIL(enc->encoder_amf, AMF_VIDEO_ENCODER_HEVC_PROFILE,
	...
	int64_t idrperiod =
		static_cast<int64_t>(ceil(keyinterv * enc->frame_rate.num));
	...
	int64_t bitrate = obs_data_get_int(settings, P_BITRATE) * 1000;
	// VBV Buffer
	SET_AMF_VALUE(enc->encoder_amf, AMF_VIDEO_ENCODER_HEVC_VBV_BUFFER_SIZE,
		      static_cast<amf_int64>(
			      bitrate *
	...
	AMF_RESULT res = enc->encoder_amf->GetProperty(
		AMF_VIDEO_ENCODER_HEVC_EXTRADATA, &p);
	if (res == AMF_OK && p.type == amf::AMF_VARIANT_INTERFACE) {
		enc->header = AMFBufferPtr(p.pInterface);
	}
	SET_AMF_VALUE(enc->encoder_amf, L"NominalRange", false);

	if (!amf_hevc_update(enc, settings))
		goto fail;
	SET_AMF_VALUE(enc->encoder_amf, AMF_VIDEO_ENCODER_HEVC_MIN_QP_I, 18);
...
void amf_hevc_defaults(obs_data_t *settings)
{
	// Preset
	...
	obs_data_set_default_int(
		settings, P_PROFILE,
		static_cast<int64_t>(AMF_VIDEO_ENCODER_HEVC_PROFILE_MAIN));
	obs_data_set_default_bool(settings, P_LOW_LATENCY_MODE, false);
	obs_data_set_default_bool(settings, P_PREPASSMODE, false);
	...
	// Picture Control
	obs_data_set_default_double(settings, P_INTERVAL_KEYFRAME, 2);
...
bool amf_hevc_properties_modified(obs_properties_t *props, obs_property_t *,
				  obs_data_t *settings)
{
	ViewMode curView =
...
bool amf_hevc_preset_modified(obs_properties_t *props, obs_property_t *,
			      obs_data_t *data)
{
	bool result = true;
	obs_property_t *p;
...
#pragma region Recording
		...
		obs_data_set_int(
			...
			static_cast<int32_t>(
				AMF_VIDEO_ENCODER_HEVC_QUALITY_PRESET_QUALITY));
		obs_data_set_int(data, P_PROFILE,
				 static_cast<int32_t>(
...
#pragma region Preset
	...
	obs_property_set_modified_callback(p, amf_hevc_preset_modified);
...
#pragma region Profile
	...
	obs_property_list_add_int(
		p, "Main",
		static_cast<int32_t>(AMF_VIDEO_ENCODER_HEVC_PROFILE_MAIN));
...
#pragma region Rate Control Method
	...
	EncoderCaps caps = AMF::Instance()->GetHEVCCaps(adapterIndex);
...
#pragma region View Mode
	...
	obs_property_set_modified_callback(p, amf_hevc_properties_modified);
...
struct obs_encoder_info obs_amf_hevc_encoder;
...
void register_amf_hevc_encoder()
{
	...
	obs_amf_hevc_encoder.id = "amf_hevc";
	obs_amf_hevc_encoder.codec = "hevc";
	obs_amf_hevc_encoder.type = OBS_ENCODER_VIDEO;
	obs_amf_hevc_encoder.caps = OBS_ENCODER_CAP_PASS_TEXTURE |
				    OBS_ENCODER_CAP_DYN_BITRATE;
	obs_amf_hevc_encoder.get_name = amf_hevc_get_name;
	obs_amf_hevc_encoder.create = amf_hevc_create;
	...
	obs_register_encoder(&obs_amf_hevc_encoder);
}
