...
AMF_RESULT init_d3d11(obs_data_t *settings, struct amf_data *enc)
{
	...
	if (FAILED(hr)) {
		PLOG_WARNING("CreateDXGIFactory1 failed");
		return AMF_FAIL;
	}
	obs_video_info info;
	...
	if (FAILED(hr)) {
		PLOG_WARNING("EnumAdapters failed");
		return AMF_FAIL;
	}
	...
	if (desc.VendorId != 0x1002) {
		PLOG_WARNING("D3D11CreateDevice failed. Invalid vendor.");
		return AMF_FAIL;
	}
	...
	hr = D3D11CreateDevice(pAdapter, D3D_DRIVER_TYPE_UNKNOWN, NULL, 0, NULL,
			       ...
			       &pD3D11Context);
	if (FAILED(hr)) {
		PLOG_WARNING("D3D11CreateDevice failed");
		return AMF_FAIL;
	}
				     ...
				     IDXGIKeyedMutex **km_out)
{
	...
	hr = device->OpenSharedResource((HANDLE)(uintptr_t)handle,
					...
					(void **)(&input_tex));
	if (FAILED(hr)) {
		PLOG_ERROR("OpenSharedResource failed");
		return NULL;
	}

	hr = input_tex->QueryInterface(_uuidof(IDXGIKeyedMutex), (void **)&km);
	if (FAILED(hr)) {
		PLOG_ERROR("QueryInterface(IDXGIKeyedMutex) failed");
		input_tex->Release();
		return NULL;
	}
		    ...
		    bool *received_packet)
{
	...
	if (handle == GS_INVALID_HANDLE) {
		PLOG_WARNING("Encode failed: bad texture handle");
		*next_key = lock_key;
		return false;
	}
	...
	if (res != AMF_OK) {
		PLOG_ERROR("Fialed to sub  with error %d\n", res);
	}
	while (true) {
		...
		PLOG_WARNING("Fialed to QueryOutput  with error %d\n", res);
...
static ID3D11Texture2D *create_texture(struct amf_data *enc)
{
	...
	if (FAILED(hr)) {
		PLOG_ERROR("Failed to create texture");
		return nullptr;
	}
...
AMF_RESULT amf_create_encoder(obs_data_t *settings, amf_data *enc)
{
	...
	// OBS Settings
	uint32_t obsWidth = obs_encoder_get_width(enc->encoder);
	uint32_t obsHeight = obs_encoder_get_height(enc->encoder);
	...
	if (result != AMF_OK) {
		PLOG_WARNING("Context Failed");
		goto clean;
	}

	result = enc->context->InitDX11(enc->pD3D11Device, AMF_DX11_1);
	if (result != AMF_OK) {
		PLOG_WARNING("Failed to init from dx11.");
		goto clean;
	}

	if (result != AMF_OK) {
		PLOG_WARNING("Failed to set converter transfer characteristic");
		goto clean;
	}
