...
namespace {
const char *MFAAC_GetName(void *)
{
	return obs_module_text("MFAACEnc");
}

obs_properties_t *MFAAC_GetProperties(void *)
{
	auto *props = obs_properties_create();

	obs_properties_add_int(props, "bitrate", obs_module_text("Bitrate"), 96, 192, 32);

	return props;
}

void MFAAC_GetDefaults(obs_data_t *settings)
{
	obs_data_set_default_int(settings, "bitrate", 128);
}

void *MFAAC_Create(obs_data_t *settings, obs_encoder_t *encoder)
{
	UINT32 bitrate = (UINT32)obs_data_get_int(settings, "bitrate");
	if (!bitrate) {
		MF_LOG_ENCODER("AAC", encoder, LOG_ERROR, "Invalid bitrate specified");
		return nullptr;
	}

	auto *audio = obs_encoder_audio(encoder);
	...
	std::unique_ptr<Encoder> enc(new Encoder(encoder, bitrate, channels, sampleRate, bitsPerSample));
...
void MFAAC_Destroy(void *data)
{
	Encoder *enc = static_cast<Encoder *>(data);
	delete enc;
}

bool MFAAC_Encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet, bool *received_packet)
{
	Encoder *enc = static_cast<Encoder *>(data);
	Status status;

	if (!enc->ProcessInput(frame->data[0], frame->linesize[0], frame->pts, &status))
		return false;
	...
	if (!enc->ProcessOutput(&outputData, &outputDataLength, &outputPts, &status))
		return false;
...
bool MFAAC_GetExtraData(void *data, uint8_t **extra_data, size_t *size)
{
	Encoder *enc = static_cast<Encoder *>(data);
