...
void MFH264_GetDefaults(obs_data_t *settings)
{
	...
	obs_data_set_default_int(settings, MFP_RATE_CONTROL, H264RateControlCBR);
	obs_data_set_default_int(settings, MFP_PROFILE, H264ProfileMain);
...
void UpdateParams(MFH264_Encoder *enc, obs_data_t *settings)
{
	...
	enc->profile = static_cast<H264Profile>(obs_data_get_int(settings, MFP_PROFILE));
	enc->rateControl = static_cast<H264RateControl>(obs_data_get_int(settings, MFP_RATE_CONTROL));
