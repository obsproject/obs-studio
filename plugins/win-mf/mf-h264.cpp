...
static bool use_advanced_modified(obs_properties_t *ppts, obs_property_t *p, obs_data_t *settings)
{
	...
	H264RateControl rateControl = (H264RateControl)obs_data_get_int(settings, MFP_RATE_CONTROL);

	if (rateControl == H264RateControlCBR || rateControl == H264RateControlVBR) {
...
static void *MFH264_Create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	enc->h264Encoder.reset(new H264Encoder(encoder, enc->descriptor, enc->width, enc->height, enc->framerateNum,
	...
	auto applySettings = [&]() {
		...
		enc.get()->h264Encoder->SetEntropyEncoding(H264EntropyEncodingCABAC);
		...
		if (enc->rateControl == H264RateControlVBR && enc->advanced && enc->useMaxBitrate)
...
static bool MFH264_Encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet, bool *received_packet)
{
	MFH264_Encoder *enc = static_cast<MFH264_Encoder *>(data);
	Status status;
	...
	*received_packet = false;
	...
	if (!enc->h264Encoder->ProcessInput(frame->data, frame->linesize, frame->pts, &status))
		return false;
...
void RegisterMFH264Encoders(const char Codec[])
{
	...
	auto encoders = EncoderDescriptor::Enumerate(Codec);
	for (auto e : encoders) {
...
void RegisterMFH264Encoders()
{
	...
	auto encoders = EncoderDescriptor::Enumerate("h264");
	for (auto e : encoders) {
		/* ignore the software encoder due to the fact that we already
		 * have an objectively superior software encoder available */
		if (e->Type() == EncoderType::H264_SOFTWARE)
...
//void RegisterMFEncoders(std::string Codec) {
//
//	if (Codec == "h264") {
//
//		obs_encoder_info info = {0};
...
//		info.codec = "h264";
//
//		auto encoders = EncoderDescriptor::Enumerate(info.codec);
//		for (auto e : encoders) {
//			/* ignore the software encoder due to the fact that we already
//		 * have an objectively superior software encoder available */
//			if (e->Type() == EncoderType::H264_SOFTWARE)
