...
static inline bool DecodeDeviceId(MediaFoundationDeviceId &out, const char *deviceId)
{
	DStr name;
	DStr path;
	...
	while (true) {
		...
		case Action::NpuControl:
			if (_mfcaptureDevice) {
				...
				int val = (int)obs_data_get_int(settings, INTELNPU_BLUR_TYPE);
				switch (val) {
				...
				mask = obs_data_get_bool(settings, INTELNPU_BACKGROUND_REMOVAL);

				bool _blur, _shallowfocus, _mask, _transparent;
				MF_GetBlur(_mfcaptureDevice, _blur, _shallowfocus, _mask);
				MF_GetTransparent(_mfcaptureDevice, _transparent);
				if (_blur != blur || _shallowfocus != shallowfocus || _transparent != mask) {
					MF_SetBlur(_mfcaptureDevice, blur, shallowfocus, mask);
				}

				bool autoframing = obs_data_get_bool(settings, INTELNPU_AUTO_FRAMING);
				bool _autoframing;
				MF_GetAutoFraming(_mfcaptureDevice, _autoframing);
				if (_autoframing != autoframing) {
					MF_SetAutoFraming(_mfcaptureDevice, autoframing);
				}

				bool eyegaze = obs_data_get_bool(settings, INTELNPU_EYEGAZE_CORRECTION);
				bool _eyegaze;
				MF_GetEyeGazeCorrection(_mfcaptureDevice, _eyegaze);
				if (_eyegaze != eyegaze) {
					MF_SetEyeGazeCorrection(_mfcaptureDevice, eyegaze);
				}
				obs_data_release(settings);
			}
			break;
...
static bool ResolutionAvailable(const MediaFoundationVideoInfo &cap, int cx, int cy)
{
...
static bool FrameRateAvailable(const MediaFoundationVideoInfo &cap, long long interval)
{
...
static inline bool MatcherClosestFrameRateSelector(long long interval, long long &best_match,
...
static inline void AddCap(vector<Resolution> &resolutions, const MediaFoundationVideoInfo &cap)
{
...
static bool TryResolution(const MediaFoundationVideoDevice &dev, const string &res)
{
	int cx;
	int cy;
...
DEFINE_GUID(OBS_DXCORE_ADAPTER_ATTRIBUTE_D3D12_GENERIC_ML, 0xb71b0d41, 0x1088, 0x4222f, 0xa2, 0x7c, 0x2, 0x50, 0xb7,
	    0xd3, 0xa9, 0x88)
...
bool NPUDetection()
{
	...
	GUID attributes[]{OBS_DXCORE_ADAPTER_ATTRIBUTE_D3D12_GENERIC_ML};
	winrt::check_hresult(
