...
void MFHEVC_GetDefaults(obs_data_t *settings)
{
	...
	obs_data_set_default_int(settings, MFP_RATE_CONTROL, HEVCRateControlCBR);
	obs_data_set_default_int(settings, MFP_PROFILE, HEVCProfileMain);
...
void UpdateParams(MFHEVC_Encoder *enc, obs_data_t *settings)
{
	...
	enc->profile = static_cast<HEVCProfile>(obs_data_get_int(settings, MFP_PROFILE));
	enc->rateControl = static_cast<HEVCRateControl>(obs_data_get_int(settings, MFP_RATE_CONTROL));
