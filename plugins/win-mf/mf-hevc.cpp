...
struct MFHEVC_Encoder {
	...
	std::unique_ptr<HEVCEncoder> hevcEncoder;
	uint32_t width;
	uint32_t height;
...
#define MFTEXT(x) obs_module_text("MF.HEVC." x)
...
static const char *MFHEVC_GetName(void *type_data)
{
...
static bool use_max_bitrate_modified(obs_properties_t *ppts, obs_property_t *p, obs_data_t *settings)
{
	UNUSED_PARAMETER(p);
...
static bool use_advanced_modified(obs_properties_t *ppts, obs_property_t *p, obs_data_t *settings)
{
	UNUSED_PARAMETER(p);
	...
	HEVCRateControl rateControl = (HEVCRateControl)obs_data_get_int(settings, MFP_RATE_CONTROL);

	if (rateControl == HEVCRateControlCBR || rateControl == HEVCRateControlVBR) {
...
static obs_properties_t *MFHEVC_GetProperties(void *)
{
	...
	obs_property_list_add_int(list, "main", HEVCProfileMain);
	...
	obs_property_list_add_int(list, TEXT_CBR, HEVCRateControlCBR);
	obs_property_list_add_int(list, TEXT_VBR, HEVCRateControlVBR);
...
static void MFHEVC_GetDefaults(obs_data_t *settings)
{
#define PROP_DEF(x, y, z) obs_data_set_default_##x(settings, y, z)
	...
	PROP_DEF(int, MFP_RATE_CONTROL, HEVCRateControlCBR);
	PROP_DEF(int, MFP_PROFILE, HEVCProfileMain);
...
static void UpdateParams(MFHEVC_Encoder *enc, obs_data_t *settings)
{
...
#define PROP_GET(x, y, z) (z) obs_data_get_##x(settings, y)
	enc->profile = PROP_GET(int, MFP_PROFILE, HEVCProfile);
	enc->rateControl = PROP_GET(int, MFP_RATE_CONTROL, HEVCRateControl);
...
static bool ApplyCBR(MFHEVC_Encoder *enc)
{
...
static bool ApplyVBR(MFHEVC_Encoder *enc)
{
	enc->hevcEncoder->SetBitrate(enc->bitrate);
...
static void *MFHEVC_Create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	enc->hevcEncoder.reset(new HEVCEncoder(encoder, enc->descriptor, enc->width, enc->height, enc->framerateNum,
...
static void MFHEVC_Destroy(void *data)
{
	MFHEVC_Encoder *enc = static_cast<MFHEVC_Encoder *>(data);
	delete enc;
}

static bool MFHEVC_Encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet, bool *received_packet)
{
	MFHEVC_Encoder *enc = static_cast<MFHEVC_Encoder *>(data);
	Status status;
	...
	*received_packet = false;
	...
	if (!enc->hevcEncoder->ProcessInput(frame->data, frame->linesize, frame->pts, &status))
		return false;
...
static bool MFHEVC_GetExtraData(void *data, uint8_t **extra_data, size_t *size)
{
	MFHEVC_Encoder *enc = static_cast<MFHEVC_Encoder *>(data);
...
static bool MFHEVC_GetSEIData(void *data, uint8_t **sei_data, size_t *size)
{
...
static void MFHEVC_GetVideoInfo(void *, struct video_scale_info *info)
{
	info->format = VIDEO_FORMAT_NV12;
}

static bool MFHEVC_Update(void *data, obs_data_t *settings)
{
	MFHEVC_Encoder *enc = static_cast<MFHEVC_Encoder *>(data);
...
void RegisterMFHEVCEncoders()
{
	...
	auto encoders = EncoderDescriptor::Enumerate("hevc");
	for (auto e : encoders) {
