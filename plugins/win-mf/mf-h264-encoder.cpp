...
namespace {
eAVEncH264VProfile MapProfile(H264Profile profile)
{
	switch (profile) {
...
eAVEncCommonRateControlMode MapRateControl(H264RateControl rc)
{
	switch (rc) {
...
UINT32 MapQpToQuality(H264QP &qp)
{
...
H264Encoder::H264Encoder(const obs_encoder_t *encoder, std::shared_ptr<EncoderDescriptor> descriptor, UINT32 width,
			 UINT32 height, UINT32 framerateNum, UINT32 framerateDen, H264Profile profile, UINT32 bitrate)
	: encoder(encoder),
...
HRESULT H264Encoder::CreateMediaTypes(ComPtr<IMFMediaType> &i, ComPtr<IMFMediaType> &o)
{
	HRESULT hr;
	...
	HRC(o->SetUINT32(MF_MT_VIDEO_PROFILE, MapProfile(profile)));
...
bool H264Encoder::SetLowLatency(bool lowLatency)
{
	...
	if (codecApi) {
		HR_CHECK(LOG_WARNING, SetCodecProperty(codecApi, CODECAPI_AVLowLatencyMode, lowLatency));
	}
...
bool H264Encoder::SetEntropyEncoding(H264EntropyEncoding entropyEncoding)
{
	...
	if (codecApi) {
		HR_CHECK(LOG_WARNING, SetCodecProperty(codecApi, CODECAPI_AVEncH264CABACEnable,
...
HRESULT H264Encoder::CreateEmptySample(ComPtr<IMFSample> &sample, ComPtr<IMFMediaBuffer> &buffer, DWORD length)
{
	HRESULT hr;
...
bool H264Encoder::ProcessInput(UINT8 **data, UINT32 *linesize, UINT64 pts, Status *status)
{
...
HRESULT H264Encoder::ProcessOutput()
{
	...
	frame.reset(new H264Frame(keyframe, samplePts / sampleDur, sampleDts / sampleDur, std::move(data)));
...
bool H264Encoder::ProcessOutput(UINT8 **data, UINT32 *dataLength, UINT64 *pts, UINT64 *dts, bool *keyframe,
				Status *status)
{
