...
void ConvertNv12ToArgb(BYTE *pDst, LONG dstStride, const BYTE *pSrc, LONG srcStride, DWORD dwWidthInPixels,
	...
	if (SUCCEEDED(hr)) {
		...
		if (pSample && m_cbVideoData && m_uiWidth && m_uiHeight) {
			...
			if (m_fmt == MF_COLOR_FORMAT_NV12) {
				...
				ConvertNv12ToArgb(m_pBufferArgbOut, m_uiWidth * 4, pbScanline0, lStride, m_uiWidth,
						  m_uiHeight);
#endif
...
#ifdef ARGB_SEG_MASK_WORKAROUND
	if (fmt == MF_COLOR_FORMAT::MF_COLOR_FORMAT_ARGB || fmt == MF_COLOR_FORMAT::MF_COLOR_FORMAT_XRGB) {
