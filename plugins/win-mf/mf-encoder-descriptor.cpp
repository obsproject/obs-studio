...
struct EncoderEntry {
	...
	{"{ADC9BC80-0F41-46C6-AB75-D693D793597D}", "MF.H264.EncoderHWAMD", "mf_h264_vce", EncoderType::H264_VCE},
	{"{4BE8D3C0-0515-4A37-AD55-E4BAE19AF471}", "MF.H264.EncoderHWIntel", "mf_h264_qsv", EncoderType::H264_QSV},
	{"{60F44560-5A20-4857-BFEF-D29773CB8040}", "MF.H264.EncoderHWNVIDIA", "mf_h264_nvenc", EncoderType::H264_NVENC},
	{"{7790EE16-08E3-426D-AADA-F96774308EA1}", "MF.H264.EncoderHWQCOM", "qcom_h264", EncoderType::H264_QCOM},
	{"{5AAFFE75-4EA4-424C-89E3-4A1E3F9A570D}", "MF.HEVC.EncoderHWQCOM", "qcom_hevc", EncoderType::HEVC_QCOM},
};
...
std::unique_ptr<EncoderDescriptor> CreateDescriptor(ComPtr<IMFActivate> activate)
{
	UINT32 flags;
	...
	EncoderEntry *entry = std::find_if(std::begin(guidNameMap), std::end(guidNameMap), pred);

	std::unique_ptr<EncoderDescriptor> descriptor(new EncoderDescriptor(
		activate, entry->name, entry->id, guid, guidString, isAsync, isHardware, entry->type));

	return descriptor;
}
} // namespace

std::vector<std::shared_ptr<EncoderDescriptor>> EncoderDescriptor::Enumerate(const char Codec[])
{
	HRESULT hr;
	UINT32 count = 0;
