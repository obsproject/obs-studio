...
#ifndef MF_LOG
#define MF_LOG(level, format, ...) blog(level, "[Media Foundation encoder]: " format, ##__VA_ARGS__)
#endif
#ifndef MF_LOG_ENCODER
#define MF_LOG_ENCODER(format_name, encoder, level, format, ...) \
	blog(level, "[Media Foundation %s: '%s']: " format, format_name, obs_encoder_get_name(encoder), ##__VA_ARGS__)
#endif

#ifndef MF_LOG_COM
#define MF_LOG_COM(level, msg, hr) MF_LOG(level, msg " failed,  %S (0x%08lx)", _com_error(hr).ErrorMessage(), hr)
#endif
