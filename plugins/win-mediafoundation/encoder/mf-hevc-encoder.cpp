...
MF::HEVCEncoder::~HEVCEncoder()
{
	...
	while (inputRequests == 0) {
		...
		if (hr != MF_E_TRANSFORM_NEED_MORE_INPUT && FAILED(hr)) {
			MF::MF_LOG_COM(LOG_ERROR, "HEVCEncoder::~HEVCEncoder: ProcessOutput()", hr);
			break;
		}
...
bool MF::HEVCEncoder::ProcessInput_Tex(ID3D11Texture2D *pSurface, int64_t pts, Status *status)
{
	ProfileScope("HEVCEncoder::ProcessInput");
