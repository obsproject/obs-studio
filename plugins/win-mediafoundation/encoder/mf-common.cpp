...
void MF::QpRange(Codec Codec, double bppf, uint32_t *minQp, uint32_t *maxQp)
{
	// Sanity
	if (!(bppf > 0.0) || !std::isfinite(bppf)) {
		// Default to a moderate content density if input is invalid
		bppf = 0.10; // ~“typical” real-time density
	}

	// Defaults (H.264-like)
	uint32_t qp_min_limit = 0;
	uint32_t qp_max_limit = 51;
	double codec_offset = 0.0;

	// Normalize codec properties
	if (Codec == QCOM_H264) {
		qp_min_limit = 0;
		qp_max_limit = 51;
		codec_offset = 0.0;
	} else if (Codec == QCOM_H265) {
		qp_min_limit = 0;
		qp_max_limit = 51;
		codec_offset = -2.0;
	} else if (Codec == QCOM_AV1) {
		qp_min_limit = 0;
		qp_max_limit = 63;
		codec_offset = -4.0;
	} else {
		// Unknown codec: be permissive.
		qp_min_limit = 0;
		qp_max_limit = 63;
		codec_offset = 0.0;
	}
	...
	// Pick α, β so that:
	//  - bppf ~ 0.30 → low QP (good quality, high bits per pixel)
	//  - bppf ~ 0.10 → mid QP
	//  - bppf ~ 0.04 → higher QP (low bits per pixel)
	//
	// α = 12, β = 4 gives reasonable real-time ranges across codecs.
	const double alpha = 12.0;
	const double beta = 4.0;

	// base QP from the model
	double base_qp_f = alpha - beta * std::log2(bppf) + codec_offset;
	uint32_t base_qp = static_cast<uint32_t>(std::lround(base_qp_f));

	// Clamp to legal range
	base_qp = std::max(qp_min_limit, std::min(base_qp, qp_max_limit));

	// Provide a window around the base QP.
	// Wider on the high side to let the encoder save bits during simple scenes.
