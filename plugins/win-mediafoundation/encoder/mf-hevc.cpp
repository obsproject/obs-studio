...
bool use_max_bitrate_modified(obs_properties_t *ppts, obs_property_t *p, obs_data_t *settings)
{
	...
	set_visible(ppts, MFP_MAX_BITRATE, use_max_bitrate);
...
{
	obs_data_set_default_int(settings, MFP_BITRATE, 6000);
	...
	obs_data_set_default_int(settings, MFP_MAX_BITRATE, 6000);
...
void *MFHEVC_Create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	auto applySettings = [&]() {
		...
		fps = (float)enc->framerateNum / (float)enc->framerateDen;
...
static bool MFTHEVC_Encode_Tex(void *data, struct encoder_texture *tex, int64_t pts, uint64_t lock_key,
