...
namespace {
eAVEncAV1VProfile MapProfile(AV1Profile profile)
{
	switch (profile) {
	case AV1ProfileMain:
...
eAVEncCommonRateControlMode MapRateControl(AV1RateControl rc)
{
	switch (rc) {
...
UINT32 MapQpToQuality(AV1QP &qp)
{
...
AV1Encoder::AV1Encoder(const obs_encoder_t *encoder, std::shared_ptr<EncoderDescriptor> descriptor, UINT32 width,
...
AV1Encoder::~AV1Encoder()
{
	...
	while (inputRequests == 0) {
		...
		if (hr != MF_E_TRANSFORM_NEED_MORE_INPUT && FAILED(hr)) {
			MF_LOG_COM(LOG_ERROR, "AV1Encoder::~AV1Encoder: ProcessOutput()", hr);
			break;
		}
...
HRESULT AV1Encoder::CreateMediaTypes(ComPtr<IMFMediaType> &i, ComPtr<IMFMediaType> &o)
{
	HRESULT hr;
...
HRESULT AV1Encoder::CreateEmptySample(ComPtr<IMFSample> &sample, ComPtr<IMFMediaBuffer> &buffer, DWORD length)
{
	HRESULT hr;
...
bool AV1Encoder::ProcessInput(UINT8 **data, UINT32 *linesize, UINT64 pts, Status *status)
{
...
bool AV1Encoder::ProcessInput_Tex(ID3D11Texture2D *pSurface, int64_t pts, Status *status)
{
	ProfileScope("AV1Encoder::ProcessInput");
...
HRESULT AV1Encoder::ProcessOutput()
{
	...
	frame.reset(new AV1Frame(keyframe, samplePts / sampleDur, sampleDts / sampleDur, std::move(data)));
...
bool AV1Encoder::ProcessOutput(UINT8 **data, UINT32 *dataLength, UINT64 *pts, UINT64 *dts, bool *keyframe,
			       Status *status)
{
