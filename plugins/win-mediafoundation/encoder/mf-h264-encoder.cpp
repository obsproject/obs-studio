...
UINT32 MapQpToQuality(MF::H264QP &qp)
{
...
MF::H264Encoder::~H264Encoder()
{
	...
	while (inputRequests == 0) {
		...
		if (hr != MF_E_TRANSFORM_NEED_MORE_INPUT && FAILED(hr)) {
			MF_LOG_COM(LOG_ERROR, "H264Encoder::~H264Encoder: ProcessOutput()", hr);
			break;
		}
...
HRESULT MF::H264Encoder::CreateMediaTypes(ComPtr<IMFMediaType> &i, ComPtr<IMFMediaType> &o)
{
	...
	CHECK_HR_ERROR(o->SetUINT32(MF_MT_VIDEO_PROFILE, MapProfile(profile)));
...
HRESULT MF::H264Encoder::DrainEvents()
{
	HRESULT hr;
...
HRESULT MF::H264Encoder::DrainEvent(bool block)
{
...
HRESULT MF::H264Encoder::InitializeEventGenerator()
{
	HRESULT hr;
...
HRESULT MF::H264Encoder::InitializeExtraData()
{
	HRESULT hr;
...
bool MF::H264Encoder::SetBitrate(UINT32 bitrate)
{
	HRESULT hr;
...
bool MF::H264Encoder::SetQP(MF::H264QP &qp)
{
	HRESULT hr;
	if (codecApi) {
		CHECK_HR_LEVEL(LOG_WARNING,
...
bool MF::H264Encoder::SetMinQP(UINT32 minQp)
{
	HRESULT hr;
...
bool MF::H264Encoder::SetMaxQP(UINT32 maxQp)
{
	HRESULT hr;
...
bool MF::H264Encoder::SetKeyframeInterval(UINT32 seconds)
{
	HRESULT hr;
...
bool MF::H264Encoder::SetMaxBitrate(UINT32 maxBitrate)
{
	HRESULT hr;
...
bool MF::H264Encoder::SetLowLatency(bool lowLatency)
{
	HRESULT hr;

	if (codecApi) {
		CHECK_HR_LEVEL(LOG_WARNING, SetCodecProperty(codecApi, CODECAPI_AVLowLatencyMode, lowLatency));
	}
...
bool MF::H264Encoder::SetBufferSize(UINT32 bufferSize)
{
	HRESULT hr;
...
bool MF::H264Encoder::SetBFrameCount(UINT32 bFrames)
{
	HRESULT hr;
...
bool MF::H264Encoder::SetEntropyEncoding(MF::H264EntropyEncoding entropyEncoding)
{
	HRESULT hr;

	if (codecApi) {
		CHECK_HR_LEVEL(LOG_WARNING, SetCodecProperty(codecApi, CODECAPI_AVEncH264CABACEnable,
...
bool MF::H264Encoder::Initialize(std::function<bool(void)> func)
{
...
bool MF::H264Encoder::ExtraData(UINT8 **data, UINT32 *dataLength)
{
	if (extraData.empty())
		return false;
...
HRESULT MF::H264Encoder::CreateEmptySample(ComPtr<IMFSample> &sample, ComPtr<IMFMediaBuffer> &buffer, DWORD length)
{
	HRESULT hr;
...
HRESULT MF::H264Encoder::EnsureCapacity(ComPtr<IMFSample> &sample, DWORD length)
{
	HRESULT hr;
...
HRESULT MF::H264Encoder::ProcessInput(ComPtr<IMFSample> &sample)
{
...
bool MF::H264Encoder::ProcessInput(UINT8 **data, UINT32 *linesize, UINT64 pts, Status *status)
{
...
bool MF::H264Encoder::ProcessInput_Tex(ID3D11Texture2D *pSurface, int64_t pts, Status *status)
{
	ProfileScope("H264Encoder::ProcessInput");
...
HRESULT MF::H264Encoder::ProcessOutput()
{
	HRESULT hr;
	ComPtr<IMFSample> sample;
	...
	auto frame = std::make_unique<MF::H264Frame>();
	bool keyframe = false;
	...
	frame.reset(new MF::H264Frame(keyframe, samplePts / sampleDur, sampleDts / sampleDur, std::move(data)));
...
bool MF::H264Encoder::ProcessOutput(UINT8 **data, UINT32 *dataLength, UINT64 *pts, UINT64 *dts, bool *keyframe,
				    Status *status)
{
