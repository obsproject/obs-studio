...
inline void TextSource::Update(obs_data_t *s)
{
	...
	if (color != new_color || opacity != new_opacity || (gradient && color2 != new_color2) ||
	    (gradient && opacity2 != new_opacity2) || gradient_dir != new_grad_dir || vertical != new_vertical ||
	    bk_color != new_bk_color || bk_opacity != new_bk_opacity || use_extents != new_extents ||
	    wrap != new_extents_wrap || extents_cx != n_extents_cx || extents_cy != n_extents_cy ||
	...
	if (old_text != text || use_outline != new_outline || outline_color != new_o_color ||
	    outline_opacity != new_o_opacity || outline_size != roundf(float(new_o_size))) {
		needs_render = true;
	}
