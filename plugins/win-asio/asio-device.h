...
#define MAX_ASIO_CLIENTS 32
struct asio_data;

struct asio_output_route {
	int mix_index; // index of the OBS track played by the device output channel (0-5 or 6 for the monitoring mix)
	int channel_index; // index of the channel of the OBS track
};
...
struct asio_device {
	...
	volatile long buffer_callbacks_in_progress;
	...
	int device_slot_index;

	/* Capture Audio (device ==> OBS) in an 'ASIO Input Source'.
	 * Each device will stream audio to a number of OBS ASIO sources acting as audio clients.
	 * An asio_data struct ptr represents a client (or ASIO Input Source).
	 */
	struct asio_data *obs_clients[MAX_ASIO_CLIENTS];
	int current_nb_clients;

	/* Output Audio (OBS ==> device) in an 'ASIO Output'.
	 ...
	 * obs_output_client: ptr to the asio_data struct representing the OBS output client.
	 * output_routes: array which holds the track index and the channel index played by each device output channel.
	 * output_frames: circular buffer which stores the frames passed to ASIO devices
	 */
	struct asio_data *obs_output_client;
	struct asio_output_route output_routes[MAX_DEVICE_CHANNELS];
	struct spsc_fifo output_frames[MAX_DEVICE_CHANNELS];
	volatile bool output_fifos_clear_requested;
};
