...
// Blacklist taken from JUCE to which I added Realtek ASIO.
...
static bool read_asio_driver_info(HKEY hKey, const char *subkey, struct asio_driver_entry *entry)
{
	...
	if (is_blacklisted_driver(subkey)) {
		infoscanner("Skipping blacklisted driver: %s", subkey);
		return false;
	}

	err = RegOpenKeyExA(hKey, subkey, 0, KEY_READ, &driverKey);
	if (err != ERROR_SUCCESS) {
		infoscanner("Failed to open subkey: %s (err=%ld)", subkey, err);
		return false;
	}
	...
	if (err != ERROR_SUCCESS) {
		infoscanner("Could not read CLSID for %s (err=%ld)", subkey, err);
		RegCloseKey(driverKey);
		return false;
	}

	if (CLSIDFromString(clsid_str, &entry->clsid) != S_OK) {
		infoscanner("CLSIDFromString failed for %s → %ls", subkey, clsid_str);
		RegCloseKey(driverKey);
		return false;
	}
	...
	if (err != ERROR_SUCCESS) {
		infoscanner("Missing Description for %s, using subkey as name", subkey);
...
struct asio_device_list *asio_device_list_create(void)
{
	...
	while (true) {
		...
		if (list->count >= MAX_NUM_ASIO_DEVICES) {
			errorscanner("Max number of drivers (%d) reached, skipping others.", MAX_NUM_ASIO_DEVICES);
			break;
		}
...
int find_free_driver_slot()
{
	...
	errorscanner("You have more than %i asio devices, which is the maximum supported by OBS.",
		     MAX_NUM_ASIO_DEVICES);
	return -1;
}
