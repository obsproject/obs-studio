 ...
 * The current host is based on JUCE ASIO host licensed under the GNU GPL v3.
 *
...
struct asio_device *asio_device_create(const char *name, CLSID clsid, int slot_number)
{
	...
	// check if the driver is in the list of current devices
	...
	if (dev->asio == NULL || dev->driver_failure) {
		...
		// initialize obs output frames and asio input track and channels info
			...
			if (dev->asio != NULL) {
				...
				if (err == ASE_OK) {
					...
					if (err = asio_device_refresh_buffer_sizes(dev) == ASE_OK) {
						...
						// check asio output ready function
...
void asio_device_open(struct asio_device *dev, double sample_rate, long buffer_size_samples)
{
	...
	// Check if the COM thread ID matches the current thread ID
	...
	// Check if the driver can report overloads
	...
	if (err == ASE_OK) {
		...
		// Set up output buffers and formats
		...
		// log the input & output audio formats
