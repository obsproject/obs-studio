...
#define ASIO_OUTPUT_FIFO_BLOCKS 8L
...
bool asio_device_clear_output_fifos(struct asio_device *dev)
{
	if (!dev || !os_atomic_load_bool(&dev->is_started)) {
		return false;
	}

	os_atomic_set_bool(&dev->output_fifos_clear_requested, true);

	for (int count = 300; count > 0; --count) {
		if (!os_atomic_load_bool(&dev->output_fifos_clear_requested)) {
			return true;
		}
...
static void asio_device_free_output_fifos(struct asio_device *dev)
{
	for (int i = 0; i < MAX_DEVICE_CHANNELS; ++i) {
		spsc_fifo_free(&dev->output_frames[i]);
	}
}

static bool asio_device_init_output_fifos(struct asio_device *dev)
{
	long block_frames = dev->current_block_size_samples;

	if (block_frames < AUDIO_OUTPUT_FRAMES) {
		block_frames = AUDIO_OUTPUT_FRAMES;
	}

	const long capacity = block_frames * ASIO_OUTPUT_FIFO_BLOCKS + 1;

	asio_device_free_output_fifos(dev);

	for (int i = 0; i < dev->total_num_output_chans; ++i) {
		if (!spsc_fifo_init(&dev->output_frames[i], capacity)) {
			asio_device_free_output_fifos(dev);
			return false;
		}
	}
...
static void asio_device_do_reset_task(void *param)
{
	...
	struct asio_data *output_client = dev->obs_output_client;
	const bool output_was_active = output_client && obs_output_active(output_client->output);
	...
	if (output_client) {
		...
		if (dev->is_open && os_atomic_load_bool(&dev->is_started)) {
			if (output_was_active) {
				os_atomic_set_bool(&dev->capture_started, true);
			} else {
				obs_output_start(output_client->output);
			}
		}
	}
	...
	if (!dev->is_open) {
		if (dev->errorstring[0] != '\0') {
			...
			error("Failed to reopen during reset");
		}
		return;
	}
...
static void asio_device_disable_buffer_processing(struct asio_device *dev)
{
	os_atomic_set_bool(&dev->is_started, false);

	while (os_atomic_load_long(&dev->buffer_callbacks_in_progress) > 0) {
		Sleep(1);
	}
}
...
{
	for (size_t i = 0; i < dev->supported_buffer_sizes.num; ++i) {
		if (dev->supported_buffer_sizes.array[i] == value) {
			return true;
		}
	}
				  ...
				  long granularity)
{
	...
	if (granularity >= 0) {
		...
		for (; size <= max_size && size <= 6400; size += step) {
			da_push_back(dev->supported_buffer_sizes, &size);
		}
	} else {
		for (int p = 0; p < 18; ++p) {
			long size = 1L << p;
	...
	if (preferred_size >= min_size && preferred_size <= max_size && !contains_size(dev, preferred_size)) {
		da_push_back(dev->supported_buffer_sizes, &preferred_size);
	}
}
...
ASIOError asio_device_refresh_buffer_sizes(struct asio_device *dev)
{
	...
	if (err == ASE_OK) {
		da_clear(dev->supported_buffer_sizes);
...
bool asio_device_init_driver(struct asio_device *dev, char *driver_error)
{
	if (!dev) {
		return false;
	}
	...
	bool init_ok = ASIO_Init(dev, GetDesktopWindow()) == ASIOTrue;
	if (!init_ok) {
	...
	/* That call seems required by misbehaving drivers although it is not mandated by the SDK. */
...
struct asio_device *asio_device_create(const char *name, CLSID clsid, int device_slot_index)
{
	...
	if (device_slot_index < 0 || device_slot_index >= MAX_NUM_ASIO_DEVICES) {
		blog(LOG_ERROR, "[ASIO] Invalid slot number %d in asio_device_create", device_slot_index);
		free(dev);
		return NULL;
	}

	strncpy(dev->device_name, name, sizeof(dev->device_name) - 1);
	if (current_asio_devices[device_slot_index] != NULL) {
		return NULL;
	}

	dev->clsid = clsid;
	dev->device_slot_index = device_slot_index;

	da_init(dev->supported_buffer_sizes);
	da_init(dev->supported_sample_rates);

	dev->callbacks.bufferSwitch = callback_sets[dev->device_slot_index].buffer_switch;
	dev->callbacks.bufferSwitchTimeInfo = callback_sets[dev->device_slot_index].buffer_switch_time_info;
	dev->callbacks.asioMessage = callback_sets[dev->device_slot_index].asio_message;
	dev->callbacks.sampleRateDidChange = callback_sets[dev->device_slot_index].sample_rate_changed;

	os_atomic_set_long(&dev->buffer_callbacks_in_progress, 0);
	...
	if (dev->asio == NULL || dev->driver_failure) {
		...
		current_asio_devices[device_slot_index] = dev;

		for (int k = 0; k < MAX_DEVICE_CHANNELS; k++) {
			dev->output_routes[k].mix_index = -1;
			dev->output_routes[k].channel_index = -1;
		}
...
void asio_device_destroy(struct asio_device *dev)
{
	...
	da_free(dev->supported_buffer_sizes);
	da_free(dev->supported_sample_rates);
...
{
	double cr = 0;
	ASIO_GetSampleRate(dev, &cr);
	return cr;
}

bool asio_device_set_sample_rate(struct asio_device *dev, double new_rate)
{
	if (!dev || !dev->asio) {
		return false;
	}
...
void asio_device_update_sample_rates_list(struct asio_device *dev)
{
	...
	bool changed = false;
	if (dev->supported_sample_rates.num != new_rates.num) {
		changed = true;
	} else {
		for (size_t i = 0; i < dev->supported_sample_rates.num; ++i) {
			if (dev->supported_sample_rates.array[i] != new_rates.array[i]) {
				changed = true;
				break;
			}
		}
	...
	if (changed) {
		da_move(dev->supported_sample_rates, new_rates);

		char buffer[256] = {0};
		for (size_t i = 0; i < dev->supported_sample_rates.num; ++i) {
			char tmp[32];
			snprintf(tmp, sizeof(tmp), "%.0f%s", dev->supported_sample_rates.array[i],
				 (i < dev->supported_sample_rates.num - 1) ? ", " : "");
...
bool asio_device_create_dummy_buffers(struct asio_device *dev)
{
	if (!dev || !dev->asio) {
		return false;
	}
	...
	for (int i = 0; i < dev->total_num_output_chans; ++i) {
		...
		if (!asio_format_init(&dev->output_format[i], chinfo.type)) {
			error("Unsupported ASIO output sample type %ld on channel %d", chinfo.type, i);
			return false;
		}
...
/* Next function exists because of bad drivers which expect some loading sequence which should be unnecessary in
 * theory. But following JUCE and for extra safety, we do it anyway 'à la Cubase'. */
void asio_device_test(struct asio_device *dev)
{
	...
	da_clear(dev->supported_buffer_sizes);
	da_clear(dev->supported_sample_rates);
		...
		if (!dev->errorstring[0]) {
			...
			if (dev->asio != NULL) {
				...
				if (err == ASE_OK) {
					...
					err = asio_device_refresh_buffer_sizes(dev);
					if (err == ASE_OK) {
						...
						if (!asio_device_create_dummy_buffers(dev)) {
						...
						Sleep(80);
						ASIO_Stop(dev);
						/* We dispose of buffers here and not in the open function because some
						 * drivers can't set samplerate while buffers are being prepared.
						 * (from a discussion with Yamaha).*/
...
void asio_device_open(struct asio_device *dev, double sample_rate, long buffer_size_samples)
{
	...
	for (size_t i = 0; i < dev->supported_sample_rates.num; ++i) {
		if (dev->supported_sample_rates.array[i] == sample_rate) {
			is_listed = true;
			break;
		}
	}
	...
	if (!asio_device_set_sample_rate(dev, temptative_rate)) {
		goto fail;
	}
	...
	if (os_atomic_load_bool(&dev->need_to_reset)) {
		...
		if (!asio_device_init_driver(dev, init_error)) {
			goto fail;
		}
	...
	if (err == ASE_OK) {
		...
		free(dev->io_buffer_space);
		dev->io_buffer_space =
			(float *)calloc(dev->current_block_size_samples * (total_buffers + 1) + 32, sizeof(float));
		if (!dev->io_buffer_space || !asio_device_init_output_fifos(dev)) {
			error("Failed to allocate ASIO output FIFOs");
			goto fail;
		}
		dev->silent_buffers = dev->io_buffer_space + dev->current_block_size_samples * total_buffers;
		...
		for (int n = 0; n < dev->total_num_input_chans; ++n) {
			...
			if (!asio_format_init(&dev->input_format[n], chinfo.type)) {
				error("Unsupported ASIO input sample type %ld on channel %d", chinfo.type, n);
				goto fail;
			}
			dev->current_bit_depth = MAX(dev->input_format[n].bit_depth, dev->current_bit_depth);
			info("input sample bitdepth: %i ", dev->current_bit_depth);
		}
		...
		for (int n = 0; n < dev->total_num_output_chans; ++n) {
			...
			if (!asio_format_init(&dev->output_format[n], chinfo.type)) {
				error("Unsupported ASIO output sample type %ld on channel %d", chinfo.type, n);
				goto fail;
			}
			dev->current_bit_depth = MAX(dev->output_format[n].bit_depth, dev->current_bit_depth);
			info("output sample bitdepth: %i ", dev->current_bit_depth);
		}
		...
		info("starting");
		os_atomic_set_bool(&dev->called_back, false);
		os_atomic_set_bool(&dev->is_started, false);
		...
		int count = 300;
		while (--count > 0 && !os_atomic_load_bool(&dev->called_back)) {
			Sleep(10);
		}
		if (!os_atomic_load_bool(&dev->called_back)) {
			error("Device didn't start correctly. No callbacks - stopping.");
			ASIO_Stop(dev);
			goto fail;
		}
		os_atomic_set_bool(&dev->is_started, true);
		dev->need_to_reset = false;
		return;
	}
fail:
	asio_device_disable_buffer_processing(dev);
	asio_device_dispose_buffers(dev);
	Sleep(20);
	dev->is_open = false;
	asio_device_close(dev);
...
void asio_device_process_buffer(struct asio_device *dev, long buffer_index)
{
	...
	for (int idx = 0; idx < MAX_ASIO_CLIENTS; ++idx) {
		...
		if (!client || !client->device_name || !os_atomic_load_bool(&client->active)) {
			continue;
		}
	...
	if (os_atomic_load_bool(&dev->output_fifos_clear_requested)) {
		for (int channel = 0; channel < dev->total_num_output_chans; ++channel) {
			spsc_fifo_clear(&dev->output_frames[channel]);
		}

		os_atomic_set_bool(&dev->output_fifos_clear_requested, false);
	}
	...
	for (int outchan = 0; outchan < dev->total_num_output_chans; ++outchan) {
		...
		bool play = os_atomic_load_bool(&dev->capture_started) &&
			    dev->output_routes[outchan].channel_index >= 0 &&
			    dev->output_routes[outchan].mix_index >= 0;
		if (play) {
			play = spsc_fifo_read(&dev->output_frames[outchan], dev->out_buffers[outchan], samps) > 0;
		}
		if (play) {
			asio_format_convert_from_float(&dev->output_format[outchan], dev->out_buffers[outchan], dst,
						       samps);
		} else {
...
void asio_device_callback(struct asio_device *dev, long index)
{
	...
	if (index >= 0 && os_atomic_load_bool(&dev->is_started)) {
		os_atomic_inc_long(&dev->buffer_callbacks_in_progress);

		if (os_atomic_load_bool(&dev->is_started)) {
		...
		os_atomic_dec_long(&dev->buffer_callbacks_in_progress);
	} else {
