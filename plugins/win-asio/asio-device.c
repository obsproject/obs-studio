...
void asio_device_process_buffer(struct asio_device *dev, long buffer_index)
{
	...
	/* input: pass audio to OBS clients and then to OBS audio pipeline */
	...
	/* output: feed ASIO buffers with OBS output, channel per channel; the obs_output selects track channels */
	if (dev->obs_output_client) {
