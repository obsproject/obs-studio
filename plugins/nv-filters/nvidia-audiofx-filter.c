...
#define TEXT_NVAFX_VAD_HINT MT_("Nvafx.VAD.Hint")
...
static void nvidia_audio_update(void *data, obs_data_t *s)
{
	...
	if (ng->nvafx_initialized) {
		...
		if ((strcmp(ng->fx, method) != 0)) {
			...
			if (!nvidia_audio_initialize_internal(data))
				nvidia_audio_disable(ng); //nvidia_audio_destroy(ng);
...
static void nvafx_logger_calback(void *data, const char *msg)
{
...
static void *nvidia_audio_create(obs_data_t *settings, obs_source_t *filter)
{
	...
	/* Setup NVIDIA logger */
	NvAFX_Status err =
		NvAFX_InitializeLogger(NVAFX_LOG_LEVEL_ERROR, LOG_TARGET_CALLBACK, NULL, &nvafx_logger_calback, ng);
	if (err != NVAFX_STATUS_SUCCESS) {
...
static obs_properties_t *nvidia_audio_properties(void *data)
{
	...
	if (ng->nvidia_sdk_dir_found) {
		...
		obs_property_set_long_description(vad, TEXT_NVAFX_VAD_HINT);
		if (!nvafx_new_sdk)
