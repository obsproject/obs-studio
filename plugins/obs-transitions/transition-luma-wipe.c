...
#define SETTINGS_LUMA_IMG "luma_image"
#define SETTINGS_LUMA_INV "luma_invert"
#define SETTINGS_LUMA_SOFT "luma_softness"

#define SETTINGS_LUMA_DESC_IMG obs_module_text("LumaWipe.Image")
#define SETTINGS_LUMA_DESC_INV obs_module_text("LumaWipe.Invert")
#define SETTINGS_LUMA_DESC_SOFT obs_module_text("LumaWipe.Softness")
...
static void luma_wipe_update(void *data, obs_data_t *settings)
{
	...
	const char *name = obs_data_get_string(settings, SETTINGS_LUMA_IMG);
	lwipe->invert_luma = obs_data_get_bool(settings, SETTINGS_LUMA_INV);
	lwipe->softness = (float)obs_data_get_double(settings, SETTINGS_LUMA_SOFT);
...
static obs_properties_t *luma_wipe_properties(void *data)
{
	...
	p = obs_properties_add_list(props, SETTINGS_LUMA_IMG,
		SETTINGS_LUMA_DESC_IMG, OBS_COMBO_TYPE_LIST,
		OBS_COMBO_FORMAT_STRING);

	obs_data_item_t *item = NULL;

	for (item = obs_data_first(lwipe->wipes_list); item;
		obs_data_item_next(&item)) {
		const char *name = obs_data_item_get_name(item);
		const char *path = obs_data_item_get_string(item);
		obs_property_list_add_string(p, name, path);
	}

	obs_properties_add_float(props, SETTINGS_LUMA_SOFT,
		SETTINGS_LUMA_DESC_SOFT, 0.0, 1.0, 0.05);
	
	obs_properties_add_bool(props, SETTINGS_LUMA_INV,
		SETTINGS_LUMA_DESC_INV);
...
{
	obs_data_set_default_string(settings, SETTINGS_LUMA_IMG,
			"linear-h.png");
	obs_data_set_default_double(settings, SETTINGS_LUMA_SOFT, 0.03);
	obs_data_set_default_bool(settings, SETTINGS_LUMA_INV, false);
}
