...
//TODO: Use plugin bundle plist value instead
NSString *const OBSVirtualCamUUID =
	[NSString stringWithFormat:@"%@", NSToString(VIRTUALCAM_DEVICE_UUID)];
...
struct virtualcam_data {
	...
	// CMIO Extension (available since macOS 12.3
...
static void virtualcam_output_raw_video(void *data, struct video_data *frame)
{
	...
	if (cmio_extension_supported()) {
		...
		OSStatus result = CMSampleBufferCreateForImageBuffer(
			kCFAllocatorDefault, frameRef, ON, NULL, NULL,
