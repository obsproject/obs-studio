...
static void vlcs_destroy(void *data)
{
	...
	if (c->play_pause_hotkey)
		obs_hotkey_unregister(c->play_pause_hotkey);
	if (c->restart_hotkey)
		obs_hotkey_unregister(c->restart_hotkey);
	if (c->stop_hotkey)
		obs_hotkey_unregister(c->stop_hotkey);
	if (c->playlist_next_hotkey)
		obs_hotkey_unregister(c->playlist_next_hotkey);
	if (c->playlist_prev_hotkey)
		obs_hotkey_unregister(c->playlist_prev_hotkey);
