...
static void do_transition(void *data, bool to_null)
{
	...
	else if (!to_null && !ss->use_cut)
		obs_transition_start(ss->transition,
				OBS_TRANSITION_MODE_AUTO,
				ss->tr_speed,
				ss->files.array[ss->cur_item].source);
	else if (to_null && !ss->use_cut)
		obs_transition_set(ss->transition, NULL);
}
...
static void ss_destroy(void *data)
{
	...
	if (ss->play_pause_hotkey)
		obs_hotkey_unregister(ss->play_pause_hotkey);

	if (ss->restart_hotkey)
		obs_hotkey_unregister(ss->restart_hotkey);

	if (ss->stop_hotkey)
		obs_hotkey_unregister(ss->stop_hotkey);

	if (ss->next_hotkey)
		obs_hotkey_unregister(ss->next_hotkey);

	if (ss->prev_hotkey)
		obs_hotkey_unregister(ss->prev_hotkey);
