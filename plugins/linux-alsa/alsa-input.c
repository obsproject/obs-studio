...
void * alsa_create(obs_data_t *settings, obs_source_t *source)
{
	...
	char *device = obs_data_get_string(settings, "device_id");
		...
		obs_property_t *p, obs_data_t *settings)
{
	UNUSED_PARAMETER(p);
	bool visible = false;
	char *device_id = obs_data_get_string(settings, "device_id");
...
obs_properties_t * alsa_get_properties(void *unused)
{
	...
	custom_pcm = obs_properties_add_text(props, "custom_pcm",
