...
struct AV1Frame {
public:
	AV1Frame(bool keyframe, UINT64 pts, UINT64 dts, std::unique_ptr<std::vector<uint8_t>> data)
		: keyframe(keyframe),
		  pts(pts),
		  dts(dts),
		  data(std::move(data))
...
private:
	AV1Frame(AV1Frame const &) = delete;
	AV1Frame &operator=(AV1Frame const &) = delete;
...
private:
	AV1Encoder(AV1Encoder const &) = delete;
	AV1Encoder &operator=(AV1Encoder const &) = delete;
...
private:
	...
	std::unique_ptr<AV1Frame> activeFrame;
	...
	// ProcessOutput yet
	std::queue<std::unique_ptr<AV1Frame>> encodedFrames;
