...
static void build_flv_meta_data(obs_output_t *context, uint8_t **output, size_t *size)
{
	...
	for (int i = 0; i < MAX_OUTPUT_VIDEO_ENCODERS; i++) {
		obs_encoder_t *vencoder_multi = obs_output_get_video_encoder2(context, i);
	...
	*enc++ = AMF_ECMA_ARRAY;
	...
	if (is_multivideotrack) {
		...
		for (int i = 0; i < MAX_OUTPUT_VIDEO_ENCODERS; i++) {
			obs_encoder_t *vencoder_multi = obs_output_get_video_encoder2(context, i);
			if (vencoder_multi && vencoder != vencoder_multi) {
				video_t *video_multi = obs_encoder_video(vencoder_multi);

				char track_id[16];
				snprintf(track_id, sizeof(track_id), "%d", i);
				enc_obj_start_val(&enc, end, track_id);

				enc_num_val(&enc, end, "width", (double)obs_encoder_get_width(vencoder_multi));
				enc_num_val(&enc, end, "height", (double)obs_encoder_get_height(vencoder_multi));
				enc_num_val(&enc, end, "videocodecid", encoder_video_codec(vencoder_multi));
				enc_num_val(&enc, end, "videodatarate", encoder_bitrate(vencoder_multi));
				enc_num_val(&enc, end, "framerate", video_output_get_frame_rate(video_multi));
