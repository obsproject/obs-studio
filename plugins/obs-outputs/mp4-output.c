...
#define do_log(level, format, ...)                                                                  \
	blog(level, "[%s output: '%s'] " format, out->muxer_flavour == FLAVOUR_MOV ? "mov" : "mp4", \
...
struct mp4_output {
	...
	enum mp4_flavour muxer_flavour;
	int flags;
...
static void *mp4_output_create_internal(obs_data_t *settings, obs_output_t *output, enum mp4_flavour flavour)
{
	struct mp4_output *out = bzalloc(sizeof(struct mp4_output));
	out->output = output;
	out->muxer_flavour = flavour;
...
{
	return mp4_output_create_internal(settings, output, FLAVOUR_MP4);
}

static void *mov_output_create(obs_data_t *settings, obs_output_t *output)
{
	return mp4_output_create_internal(settings, output, FLAVOUR_MOV);
}
...
static bool mp4_output_start(void *data)
{
	...
	out->muxer = mp4_mux_create(out->output, &out->serializer, out->flags, out->muxer_flavour);
