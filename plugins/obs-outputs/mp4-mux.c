...
#define do_log(level, format, ...)                                                           \
	blog(level, "[%s muxer: '%s'] " format, mux->flavour == FLAVOUR_MOV ? "mov" : "mp4", \
...
static size_t mp4_write_free(struct mp4_mux *mux)
{
	...
	s_wb32(s, 16);
	s_write(s, mux->flavour == FLAVOUR_MOV ? "wide" : "free", 4);
	s_wb64(s, 0);
...
static size_t mp4_write_mdhd(struct mp4_mux *mux, struct mp4_track *track)
{
	...
	s_wb16(s, mux->flavour == FLAVOUR_MOV ? 32767 : 21956); // language (undefined)
...
{
	struct serializer *s = mux->serializer;
	bool is_mov = mux->flavour == FLAVOUR_MOV;
...
static size_t mp4_write_stsd(struct mp4_mux *mux, struct mp4_track *track)
{
	...
	if (track->type == TRACK_AUDIO && mux->flavour != FLAVOUR_MOV) {
		audio_t *audio = obs_encoder_audio(track->encoder);
...
static size_t mp4_write_udta(struct mp4_mux *mux)
{
	...
	if (mux->flavour == FLAVOUR_MOV && !(mux->flags & MP4_USE_MDTA_KEY_VALUE)) {
			       ...
			       enum mp4_flavour flavour)
{
	...
	if (flavour == FLAVOUR_MOV && mux->creation_time > UINT32_MAX) {
