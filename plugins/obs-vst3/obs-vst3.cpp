...
// global VST3Scanner
VST3Scanner *list;
std::atomic<bool> vst3_scan_done = false;
...
bool retrieve_vst3_list()
{
	...
	// The VST3 enumeration can take several seconds slowing down OBS startup.
	// So we do it in its own thread.
	std::thread([] {
...
static inline obs_source_t *get_sidechain(struct vst3_audio_data *vd)
{
	if (vd->weak_sidechain)
		return obs_weak_source_get_source(vd->weak_sidechain);
	return NULL;
}
...
static void vst3_destroy(void *data)
{
	...
	if (vd->weak_sidechain) {
		obs_source_t *sidechain = get_sidechain(vd);
		if (sidechain) {
...
static void destroy_current_VST3Plugin(vst3_audio_data *vd, obs_data *settings)
{
	...
	vd->bypass.store(true, std::memory_order_relaxed);
	vd->plugin->setProcessing(false);
	vd->plugin->hideEditor();
	vd->plugin->deactivateComponent();
...
static bool create_VST3Plugin(vst3_audio_data *vd)
{
	...
	if (!raw->init(class_id, vst3_path, sample_rate, max_block, arr)) {
		infovst3("Failed to initialize VST3 plugin at: %s", vst3_path.c_str());
	...
	vd->plugin = std::shared_ptr<VST3Plugin>(raw, [](VST3Plugin *p) {
		if (p) {
			p->deleteLater();
		}
	});
...
static void vst3_update(void *data, obs_data_t *settings)
{
	...
	bool initial_load = vd->vst3_id.empty() && !vd->plugin;
	...
	if (is_swap) {
		...
		if (!list->get_path_by_id(vst3_plugin_id).empty()) {
			vd->vst3_path = list->get_path_by_id(vst3_plugin_id);
			vd->vst3_name = list->get_name_by_id(vst3_plugin_id);
		} else {
		...
		if (init_VST3Plugin(vd, settings)) {
			vd->plugin->setProcessing(true);
			vd->bypass.store(false, std::memory_order_relaxed);
			// we support sidechain only for mono or stereo buses (sanity check)
			vd->sc_channels = vd->plugin->sidechainNumChannels;
	...
	if (vd->plugin && initial_load) {
		...
		if (hexComp && *hexComp) {
			...
			vd->plugin->loadStates(comp, ctrl);
		}
	}
...
static void *vst3_create(obs_data_t *settings, obs_source_t *filter)
{
	...
	for (size_t i = 0; i < channels; i++) {
		deque_reserve(&vd->input_buffers[i], frames * sizeof(float));
		deque_reserve(&vd->output_buffers[i], frames * sizeof(float));
		deque_reserve(&vd->sc_input_buffers[i], frames * sizeof(float));
	}
...
void vst3_save(void *data, obs_data_t *settings)
{
	...
	if (vd && vd->plugin) {
		...
		if (vd->plugin->saveStates(comp, ctrl)) {
...
static inline void preprocess_input(struct vst3_audio_data *vd, const std::shared_ptr<VST3Plugin> &plugin)
{
	...
	if (has_sc && sc_enabled) {
		...
		if (needs_resampling && vd->sc_resampler) {
			uint8_t *resampled[2] = {nullptr, nullptr};
			uint32_t out_frames;
			uint64_t ts_offset;
			auto r = vd->sc_resampler;
			if (audio_resampler_resample(r.get(), resampled, &out_frames, &ts_offset,
		...
		if (sidechain) {
			...
			if (needs_resampling) {
				...
				if (!raw) {
					...
					vd->sc_resampler =
						std::shared_ptr<audio_resampler>(raw, [](audio_resampler *r) {
							if (r)
