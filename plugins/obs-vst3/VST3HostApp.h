...
public:
	explicit VST3HostApp(VST3Backend backend);
	~VST3HostApp() noexcept;

	VST3Backend backend() const noexcept { return backend_; }

	VST3HostApp(const VST3HostApp &) = delete;
	VST3HostApp &operator=(const VST3HostApp &) = delete;
	VST3HostApp(VST3HostApp &&) = delete;
	VST3HostApp &operator=(VST3HostApp &&) = delete;

	tresult PLUGIN_API getName(String128 name) override;
	tresult PLUGIN_API createInstance(TUID cid, TUID iid_, void **obj) override;
	tresult PLUGIN_API isPlugInterfaceSupported(const TUID iid_) override;

	tresult PLUGIN_API queryInterface(const TUID iid_, void **obj) override;
...
#endif
private:
	const VST3Backend backend_;

	std::vector<FUID> FUIDArray_;
	void addPlugInterfaceSupported(const TUID iid_) { FUIDArray_.push_back(FUID::fromTUID(iid_)); }
};
