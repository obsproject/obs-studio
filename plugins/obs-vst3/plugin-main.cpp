...
extern bool loadHost();
extern void unloadHost();
extern std::atomic<bool> vst3ScanDone;
extern bool retrieveVst3List();
extern void freeVst3List();
...
{
	for (int i = 0; i < 50 && !vst3ScanDone.load(std::memory_order_acquire); ++i) {
		os_sleep_ms(100);
	}
}
