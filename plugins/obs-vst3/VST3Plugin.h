 ...
 * them using a singleton host context while we use separate host contexts for each plugin to better insulate VST3s.
 * */
...
public:
	...
	// Raw VST3 pointers: plugProvider owns the real refcounted IPtr refs. Raw ptrs avoid buggy plugin refcounting
	// and Linux teardown issues related to the runloop. A DAW would need shared refs across subsystems. But OBS is
	// a single-client host with an atomic pipeline, so a simpler raw-ptr approach is safer and fully sufficient.
