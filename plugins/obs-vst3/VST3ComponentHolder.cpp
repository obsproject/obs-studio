...
tresult PLUGIN_API VST3ComponentHolder::restartComponent(int32 flags)
{
	...
	if (flags & kLatencyChanged) {
		plugin->filterData->bypass.store(true, std::memory_order_relaxed);
		...
		plugin->filterData->bypass.store(false, std::memory_order_relaxed);
		handled = true;
	}
	if (flags & kParamValuesChanged) {
		if (plugin->editController) {
			int32 count = plugin->editController->getParameterCount();
			for (int32 i = 0; i < count; ++i) {
				Steinberg::Vst::ParameterInfo info{};
				if (plugin->editController->getParameterInfo(i, info) == kResultOk) {
					ParamValue value = plugin->editController->getParamNormalized(info.id);
	...
	return handled ? kResultOk : kNotImplemented;
}
