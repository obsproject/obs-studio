...
- (NSSize)windowWillResize:(NSWindow *)sender toSize:(NSSize)frameSize
{
    ...
    const int wantedWidth = std::max(0, (int) contentRect.size.width);
    const int wantedHeight = std::max(0, (int) contentRect.size.height);

    Steinberg::ViewRect constrained(0, 0, wantedWidth, wantedHeight);
    self.vst3View->checkSizeConstraint(&constrained);

    NSRect constrainedContentRect = NSMakeRect(0, 0, constrained.getWidth(), constrained.getHeight());
    NSRect constrainedFrameRect = [sender frameRectForContentRect:constrainedContentRect];

    return constrainedFrameRect.size;
}
- (void)windowDidResize:(NSNotification *)notification
{
    ...
    if (self.vst3View) {
        Steinberg::ViewRect vr(0, 0, (Steinberg::int32) size.width, (Steinberg::int32) size.height);
...
{
    NSWindow *window = (NSWindow *) notification.object;
    if (!self.vst3View || !window)
        return;

    Steinberg::FUnknownPtr<Steinberg::IPlugViewContentScaleSupport> scaleSupport(self.vst3View);
    if (scaleSupport)
        scaleSupport->setContentScaleFactor([window backingScaleFactor]);
}
@end
...
class PlugFrameImpl : public Steinberg::IPlugFrame
{
    ...
    {
        if (!window_ || !view || !newSize)
            return Steinberg::kInvalidArgument;

        Steinberg::ViewRect constrained = *newSize;
        view->checkSizeConstraint(&constrained);

        [window_ setContentSize:NSMakeSize(constrained.getWidth(), constrained.getHeight())];
...
class VST3EditorWindow::Impl
{
    ...
    bool create(int width, int height)
    {
        ...
        Steinberg::ViewRect initialRect(0, 0, width, height);
        if (view_)
            view_->checkSizeConstraint(&initialRect);

        NSRect contentRect = NSMakeRect(0, 0, initialRect.getWidth(), initialRect.getHeight());
        ...
        if (view_) {
            ...
            Steinberg::ViewRect vr(0, 0, initialRect.getWidth(), initialRect.getHeight());
            if (view_->getSize(&rect) == Steinberg::kResultOk &&
                (rect.getWidth() != vr.getWidth() || rect.getHeight() != vr.getHeight())) {
                Steinberg::ViewRect constrained = rect;
                view_->checkSizeConstraint(&constrained);
                [window_ setContentSize:NSMakeSize(constrained.getWidth(), constrained.getHeight())];
                view_->onSize(&constrained);
            } else {
                view_->onSize(&vr);
            }
