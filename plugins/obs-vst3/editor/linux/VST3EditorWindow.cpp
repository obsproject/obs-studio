...
void send_xembed_message(Display *dpy, Window w, Atom messageType, long message, long detail, long data1, long data2)
{
	XEvent ev;
...
public:
	...
	~Impl()
	{
		if (window_ && display_) {
			if (pluginWindow_) {
				runLoop_->unregisterWindow(pluginWindow_);
				XUnmapWindow(display_, pluginWindow_);
				XReparentWindow(display_, pluginWindow_, RootWindow(display_, DefaultScreen(display_)),
						0, 0);
		...
		view_ = nullptr;
		delete resizeDebounceTimer_;
	}

	void setClient(Window client)
	{
		if (client != 0) {
			pluginWindow_ = client;
			...
			runLoop_->registerWindow(pluginWindow_,
						 [this](const XEvent &e) { return handleMainWindowEvent(e); });
			XResizeWindow(display_, pluginWindow_, currentSize_.width, currentSize_.height);
			...
			XGetWindowAttributes(display_, window_, &parentAttr);
			XGetWindowAttributes(display_, pluginWindow_, &clientAttr);

			if (parentAttr.colormap != clientAttr.colormap) {
				fprintf(stderr, "WARNING: Colormap mismatch; fixing\n");
				XSetWindowColormap(display_, window_, clientAttr.colormap);
			}
			...
			send_xembed_message(display_, pluginWindow_, xEmbedAtom, XEMBED_EMBEDDED_NOTIFY, 0, window_, 0);

			if (xEmbedInfoAtom == None)
				xEmbedInfoAtom = XInternAtom(display_, "_XEMBED_INFO", False);

			struct XEmbedInfo info{0, 0};
			XChangeProperty(display_, pluginWindow_, xEmbedInfoAtom, xEmbedInfoAtom, 32, PropModeReplace,
					reinterpret_cast<const unsigned char *>(&info), 2);

			XMapWindow(display_, pluginWindow_);
			XSync(display_, False);
			XFlush(display_);
		}
	}
	...
	{
		bool handled = false;
		if (event.xany.window == pluginWindow_ && pluginWindow_ != 0) {
			switch (event.type) {
			case Expose:
				/* DEBUG
				 if (view_) {
					fprintf(stderr, "[vst3] Expose event received by pluginWindow_\n");
				}*/
				return true;
			case PropertyNotify:
			...
			case ConfigureNotify:
				if (resizeable_) {
					...
					XGetWindowAttributes(display_, window_, &hostAttr);
					XWindowAttributes attr;
					XGetWindowAttributes(display_, pluginWindow_, &attr);
					if (attr.width != hostAttr.width || attr.height != hostAttr.height)
						XResizeWindow(display_, window_, attr.width, attr.height);
				}
				return true;
			case ClientMessage: {
				/* DEBUG:
				...
				if (event.xclient.message_type == xEmbedAtom) {
					...
					case XEMBED_REQUEST_FOCUS: {
						XSetInputFocus(display_, pluginWindow_, RevertToParent, CurrentTime);
						send_xembed_message(display_, pluginWindow_, xEmbedAtom,
								    XEMBED_FOCUS_IN, 0, window_, 0);
						return true;
					}
					default:
						break;
		...
		} else if (event.xany.window == window_ && window_ != 0) {
			switch (event.type) {
			case MapNotify:
			case VisibilityNotify:
			case Expose:
				if (pluginWindow_) {
					XClearArea(display_, pluginWindow_, 0, 0, 0, 0, True);
					XFlush(display_);
				}
				return false;
			case ClientMessage: {
				...
				if ((Atom)event.xclient.message_type == wmProtocols_ &&
				    (Atom)event.xclient.data.l[0] == wmDelete_) {
					close();
					wasClosed_ = true;
					return true;
				}
			...
			case UnmapNotify:
				if (event.xunmap.window == window_) {
					close();
					wasClosed_ = true;
					return false;
				}
				break;
			case ReparentNotify:
				if (event.xreparent.parent == window_ && event.xreparent.window != pluginWindow_) {
			...
			case CreateNotify:
				if (event.xcreatewindow.parent != event.xcreatewindow.window &&
				    event.xcreatewindow.parent == window_ &&
				    event.xcreatewindow.window != pluginWindow_) {
	...
	bool create(int width, int height)
	{
		...
		window_ = XCreateWindow(display_, RootWindow(display_, screen), 0, 0, width, height, 0,
					DefaultDepth(display_, screen), InputOutput, DefaultVisual(display_, screen),
					mask, &winAttr);

		runLoop_->registerWindow(window_, [this](const XEvent &event) { return handleMainWindowEvent(event); });
		...
		XChangeProperty(display_, window_, windowType, XA_ATOM, 32, PropModeReplace,
				(unsigned char *)&normalType, 1);
		...
		XChangeProperty(display_, window_, allowedActions, XA_ATOM, 32, PropModeReplace,
				(unsigned char *)actions, 3);

		XMapWindow(display_, window_);
		XFlush(display_);

		frame_ = new PlugFrameImpl(display_, window_, this);
		if (view_) {
			view_->setFrame(frame_);
			if (view_->attached((void *)window_, Steinberg::kPlatformTypeX11EmbedWindowID) !=
			    Steinberg::kResultOk) {
				view_->setFrame(nullptr);
				return false;
			}
			attached_ = true;
			int _width = currentSize_.width;
			int _height = currentSize_.height;
	...
	{
		view_->setFrame(frame_);
		Steinberg::tresult res = view_->attached((void *)window_, Steinberg::kPlatformTypeX11EmbedWindowID);
	...
	void show()
	{
		if (window_ && display_) {
			if (!attached_) {
				reattach();
				if (pluginWindow_) {
					setClient(pluginWindow_);
					XReparentWindow(display_, pluginWindow_, window_, 0, 0);
				}
			}
			...
			if (attached_) {
				...
				if (view_->getSize(&cur) == Steinberg::kResultTrue)
					view_->onSize(&cur);
				currentSize_.width = cur.getWidth();
				currentSize_.height = cur.getHeight();
				XMapWindow(display_, window_);
				XMapWindow(display_, pluginWindow_);
	...
	void detach()
	{
		...
		if (pluginWindow_) {
			runLoop_->unregisterWindow(pluginWindow_);
			XUnmapWindow(display_, pluginWindow_);
			XReparentWindow(display_, pluginWindow_, RootWindow(display_, DefaultScreen(display_)), 0, 0);
			XSync(display_, False);
		}
	...
	{
		if (!force && currentSize_ == newSize)
			return;
		if (window_)
			XResizeWindow(display_, window_, newSize.width, newSize.height);
	...
	void handleDebouncedResize()
	{
		...
		if (constraintSize != currentSize_) {
			currentSize_ = constraintSize;
			onResize(constraintSize);
		}

		if (pluginWindow_)
			XResizeWindow(display_, pluginWindow_, constraintSize.width, constraintSize.height);
	...
	public:
		PlugFrameImpl(Display *display, Window win, Impl *owner)
			: display_(display),
			  window_(win),
			  owner_(owner)
		{
							 ...
							 Steinberg::ViewRect *newSize) override
		{
			if (display_ && newSize) {
				...
				XResizeWindow(display_, window_, newSize->getWidth(), newSize->getHeight());
				if (owner_->pluginWindow_)
					XResizeWindow(display_, owner_->pluginWindow_, newSize->getWidth(),
						      newSize->getHeight());
				view->onSize(newSize);
