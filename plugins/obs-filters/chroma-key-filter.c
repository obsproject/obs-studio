...
static void *chroma_key_create(obs_data_t *settings, obs_source_t *context)
{
	...
	if (version == 1)
		effect_path = obs_module_file("chroma_key_filter.effect");
	else if (version == 2)
		effect_path = obs_module_file("chroma_key_filter_v2.effect");
...
static obs_properties_t *chroma_key_properties(uint32_t version)
{
...
static obs_properties_t *chroma_key_properties_v1(void *data)
{
	...
	return chroma_key_properties(1);
}

static obs_properties_t *chroma_key_properties_v2(void *data)
{
	UNUSED_PARAMETER(data);
	return chroma_key_properties(2);
}
