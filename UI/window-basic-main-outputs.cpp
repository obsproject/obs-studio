...
inline BasicOutputHandler::BasicOutputHandler(OBSBasic *main_) : main(main_)
{
	if (main->vcamEnabled) {
		...
		QObject::connect(virtualCam.data(), &OutputObj::OutputStarted, main, &OBSBasic::OnVirtualCamStart);
		QObject::connect(virtualCam.data(), &OutputObj::OutputStopped, main, &OBSBasic::OnVirtualCamStop);
	}
...
void BasicOutputHandler::UpdateVirtualCamOutputSource()
{
	...
	case VCamOutputType::PreviewOutput: {
		...
		source = obs_source_get_ref(main->GetCurrentSceneSource());
		break;
	}
