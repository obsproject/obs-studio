...
void OBSSceneCollections::on_lineeditSearch_textChanged(const QString &text)
{
	...
	for (auto row : ui->listCollections->rows()) {
...
void OBSSceneCollections::setBulkMode(bool bulk)
{
	...
	for (auto child : ui->listCollections->rows()) {
...
static QString format_relative_time(time_t from)
{
	...
	/* This is quite ugly. */
	if (diff < MINUTE) {
		return QTStr("LastUsed.JustNow");
	} else if (diff < 2 * MINUTE) {
		return QTStr("LastUsed.Minute").arg(diff / MINUTE);
	} else if (diff < HOUR) {
		return QTStr("LastUsed.Minutes").arg(diff / MINUTE);
	} else if (diff < 2 * HOUR) {
		return QTStr("LastUsed.Hour").arg(diff / HOUR);
	} else if (diff < DAY) {
		return QTStr("LastUsed.Hours").arg(diff / HOUR);
	} else if (diff < 2 * DAY) {
		return QTStr("LastUsed.Day").arg(diff / DAY);
	} else if (diff < WEEK) {
		return QTStr("LastUsed.Days").arg(diff / DAY);
	} else if (diff < 2 * WEEK) {
		return QTStr("LastUsed.Week").arg(diff / WEEK);
	} else if (diff < MONTH) {
		return QTStr("LastUsed.Weeks").arg(diff / WEEK);
	} else if (diff < 2 * MONTH) {
		return QTStr("LastUsed.Month").arg(diff / MONTH);
	} else if (diff < YEAR) {
		return QTStr("LastUsed.Months").arg(diff / MONTH);
	} else if (diff < 2 * YEAR) {
		return QTStr("LastUsed.Year").arg(diff / YEAR);
	} else {
		return QTStr("LastUsed.Years").arg(diff / YEAR);
	}
}
...
void OBSSceneCollections::refreshList()
{
	...
	EnumSceneCollectionsOrdered(collections_order, [&](const char *name,
		...
		QString description = isCurrentCollection
					      ? QTStr("LastUsed.CurrentlyOpen")
					      : format_relative_time(last_used);
		OBSActionRow *row = new OBSActionRow(name, description,
						     ui->listCollections);
