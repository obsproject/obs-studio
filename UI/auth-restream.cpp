...
class RestreamWidget : public OBSDock {
public:
	inline RestreamWidget() : OBSDock() {}
...
void RestreamAuth::LoadUI()
{
	...
	QCefWidget *browserChat;
	std::string urlChat;
	//std::string script;

	// Chat widget
	urlChat = "https://restream.io/chat-application";
	...
	browserChat = cef->create_widget(nullptr, urlChat, panel_cookies);
	chat->setWidget(browserChat);
	...
	QCefWidget *browserInfo;
	std::string urlInfo;

	urlInfo = "https://restream.io/titles/embed";
	...
	browserInfo = cef->create_widget(nullptr, urlInfo, panel_cookies);
	info->setWidget(browserInfo);
