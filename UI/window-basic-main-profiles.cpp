...
using CheckResult = std::optional<std::pair<QStringView, QString>>;
...
static CheckResult CheckNVENCInternal(std::unordered_set<std::string_view> &missing_encoders)
{
	...
	if (config_has_user_value(config, "general", "reason")) {
		...
		for (auto &[code, desc] : nvencFailureReasons) {
			...
			QString ret =
				QTStr("EncoderMissing.NVENC.TestProgramError").arg(QTStr(desc.data())).arg(code.data());
	...
	if (includes_av1 && !config_get_bool(config, "av1", "codec_supported"))
		return std::make_pair(u"no_av1", QTStr("EncoderMissing.NVENC.Reason.AV1Unsupported"));
...
static CheckResult CheckX264()
{
	...
	if (!module_loaded) {
		return std::make_pair(u"plugin_not_loaded", QTStr("EncoderMissing.MissingModule").arg("obs-x264"));
	}
