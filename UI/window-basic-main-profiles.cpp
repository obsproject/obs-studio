...
const OBSProfile &OBSBasic::CreateProfile(const std::string &profileName)
{
	...
	profileDirectory.reserve(App()->userProfilesLocation.size() +
				 OBSProfilePath.size() + directoryName.size());
	profileDirectory.append(App()->userProfilesLocation)
	...
	const std::filesystem::path profileDirectoryPath{profileDirectory};
	...
	const std::filesystem::path profileFile{profileDirectoryPath /
						OBSProfileSettingsFile.data()};
...
void OBSBasic::RefreshProfileCache()
{
	...
	const std::filesystem::path profilesPath{App()->userProfilesLocation +
						 OBSProfilePath.data()};
	...
	for (const auto &entry :
		...
		const std::filesystem::path profileCandidate{
			entry.path() / OBSProfileSettingsFile};
...
void OBSBasic::on_actionImportProfile_triggered()
{
	...
	if (!sourceDirectory.isEmpty() && !sourceDirectory.isNull()) {
		...
		const std::filesystem::path destinationPath{
			...
			"/obs-studio/basic/profiles/" + directoryName};
