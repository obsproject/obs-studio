...
void OBSBasic::on_actionImportProfile_triggered()
{
	...
	if (!dir.isEmpty() && !dir.isNull()) {
		...
		if (!folder.exists()) {
			...
			QFile::copy(dir+
				"/basic.ini", profileDir+"/basic.ini");
			QFile::copy(dir+
				"/service.json", profileDir+"/service.json");
			RefreshProfiles();
		} else {
...
void OBSBasic::on_actionExportProfile_triggered()
{
	...
	if (!dir.isEmpty() && !dir.isNull()) {
		...
		if (!folder.exists()) {
			...
			QFile::copy(inputPath+currentProfile+
				"/basic.ini",outputDir+"/basic.ini");
			QFile::copy(inputPath+currentProfile+
				"/service.json",outputDir+"/service.json");
		}
	}
}
