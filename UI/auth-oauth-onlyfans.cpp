	  ...
	  of_base_url(base_url)
{
	...
	connect(this->cefWidget, SIGNAL(titleChanged(const QString &)), this,
		SLOT(setWindowTitle(const QString &)));
	connect(this->cefWidget, SIGNAL(urlChanged(const QString &)), this,
	...
	for (const auto &param : params) {
		...
		if (next_idx != -1) {
			if (strcmp(param, "accessToken=") == 0) {
				this->token =
					url.mid(code_idx, next_idx - code_idx);
			} else {
				this->refresh_token =
					url.mid(code_idx, next_idx - code_idx);
			}
...
//-------------------------------------------------------------------------//
static std::string get_config_str(OBSBasic *main, const char *section,
				  const char *name)
{
...
//-------------------------------------------------------------------------//
auto OAuthOnlyfansStreamKey::GetToken(const char *url,
				      const std::string &auth_token) -> bool
{
	return GetTokenInternal(url, auth_token);
}

auto OAuthOnlyfansStreamKey::GetToken(const std::string &url,
				      const std::string &auth_token) -> bool
{
	return GetTokenInternal(url.c_str(), auth_token);
}
//-------------------------------------------------------------------------//
auto OAuthOnlyfansStreamKey::GetTokenInternal(const char *url,
	...
	try {
		...
		if (this->currentScopeVer > 0) {
			if (this->RetryLogin()) {
				...
				QString title =
					QTStr("Auth.InvalidScope.Title");
				QString text = QTStr("Auth.InvalidScope.Text")
						       .arg(service());

				QMessageBox::warning(OBSBasic::Get(), title,
						     text);
			}
		}
		...
		if (not success || output.empty()) {
			throw ErrorInfo("Failed to get token from remote",
					error);
		}
...
auto OAuthOnlyfansStreamKey::OnStreamConfig() -> void
{
	...
	if (bwtest && strcmp(this->service(), "OnlyFans.com") == 0) {
		obs_data_set_string(
			settings, "key",
			(this->key_ + "?bandwidthtest=true").c_str());
	} else {
