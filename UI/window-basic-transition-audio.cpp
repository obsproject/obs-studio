...
OBSBasicTransitionAudio::OBSBasicTransitionAudio(QWidget *parent,
						 OBSSource source_)
	: QDialog(parent),
	  ...
	  source(source_)
{
	...
	setWindowTitle(
		QTStr("%1 - %2").arg(QTStr("Basic.AdvAudio"), sourceName));
...
void OBSBasicTransitionAudio::OBSSourceUpdated(void *param,
					       calldata_t *calldata)
{
	QMetaObject::invokeMethod(
		reinterpret_cast<OBSBasicTransitionAudio *>(param),
		"SourceUpdated");
...
static inline bool checkSettingExists(obs_data_t *settings,
				      const char *setting_name)
{
	return (obs_data_has_user_value(settings, setting_name) ||
		obs_data_has_default_value(settings, setting_name));
}
...
void OBSBasicTransitionAudio::SourceUpdated()
{
	...
	float volumeVal =
		checkSettingExists(settings, "audio_volume")
	...
	int balanceVal =
		checkSettingExists(settings, "audio_balance")
			? (int)(obs_data_get_double(settings, "audio_balance") *
				100.0l)
			: 50;
	...
	int monitoringType =
		checkSettingExists(settings, "audio_monitoring")
	...
	uint32_t mixers =
		checkSettingExists(settings, "audio_mixers")
...
void OBSBasicTransitionAudio::monitoringTypeChanged(int index)
{
	...
	const char *name = obs_source_get_name(source);
	blog(LOG_INFO,
	     "User changed audio monitoring for transition '%s' to: %s",
	     name ? name : "(null)", type);
