...
bool OBSBasic::InitBasicConfig()
{
	...
	try {
		...
		profileLocation.reserve(App()->userProfilesLocation.size() +
					OBSProfilePath.size() +
					profileDirectory.size());
		profileLocation.append(App()->userProfilesLocation)
			.append(OBSProfilePath)
			.append(profileDirectory);
		const std::filesystem::path profilePath{profileLocation};
		...
		const std::filesystem::path userProfileFile{
			profilePath / OBSProfileSettingsFile.data()};

		if (userProfileFile.empty()) {
		...
		if (config_get_string(activeConfiguration, "General", "Name") ==
		    nullptr) {
			const char *curName = config_get_string(
				App()->GetUserConfig(), "Basic", "Profile");

			config_set_string(activeConfiguration, "General",
					  "Name", curName);
			activeConfiguration.SaveSafe("tmp");
		}
...
void OBSBasic::on_actionShowSettingsFolder_triggered()
{
	const std::filesystem::path userConfigPath{
		std::filesystem::path{App()->userConfigLocation} /
		"obs-studio"};
	const QString userConfigLocation =
		QString::fromStdString(userConfigPath.string());
...
void OBSBasic::on_actionShowProfileFolder_triggered()
{
	...
	const QString userProfileLocation =
		QString::fromStdString(userProfilePath.string());
