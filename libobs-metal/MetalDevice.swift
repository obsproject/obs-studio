...
class MetalDevice {
    ...
    private var obsSignalCallbacks = [String: () -> Void]()
    ...
    func dispatchSignal(type: MetalSignalType) {
        if let callback = obsSignalCallbacks[type.rawValue] {
            callback()
        }
    }
    ...
    private func setupSignalHandlers() {
        ...
        obsSignalCallbacks.updateValue(videoResetCallback, forKey: MetalSignalType.videoReset.rawValue)
    }
