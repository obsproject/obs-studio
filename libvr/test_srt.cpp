...
int main()
{
	...
	ITransportAdapter *adapter = CreateSRTAdapter();
	if (!adapter) {
		std::cerr << "[TestSRT] Adapter creation failed (libsrt missing?). Skipping test." << std::endl;
		return 0;
	}
	...
	if (adapter->vtbl->Initialize) {
		...
		std::cout << "[TestSRT] Initialize called (might fail due to no listener, that's expected)."
			  << std::endl;
	}
