...
static void AddInputLayoutVar(shader_var *var, std::vector<D3D12_INPUT_ELEMENT_DESC> &layout)
{
...
static inline bool SetSlot(std::vector<D3D12_INPUT_ELEMENT_DESC> &layout, const char *name, uint32_t index,
			   uint32_t &slotIdx)
{
	for (size_t i = 0; i < layout.size(); i++) {
		D3D12_INPUT_ELEMENT_DESC &input = layout[i];
...
static void BuildInputLayoutFromVars(shader_parser *parser, darray *vars, std::vector<D3D12_INPUT_ELEMENT_DESC> &layout)
{
...
void ShaderProcessor::BuildInputLayout(std::vector<D3D12_INPUT_ELEMENT_DESC> &layout)
{
...
static inline void AddParam(shader_var &var, std::vector<gs_shader_param> &params, uint32_t &texCounter)
{
...
void ShaderProcessor::BuildSamplers(std::vector<std::unique_ptr<ShaderSampler>> &samplers)
{
