...
{
	return autoConfig()->type == AutoConfig::Type::Recording ? AutoConfig::TestPage : AutoConfig::StreamPage;
}

bool AutoConfigVideoPage::validatePage()
{
	int encRes = ui->canvasRes->currentData().toInt();
	autoConfig()->baseResolutionCX = encRes >> 16;
	autoConfig()->baseResolutionCY = encRes & 0xFFFF;
	autoConfig()->fpsType = (AutoConfig::FPSType)ui->fps->currentData().toInt();

	obs_video_info ovi;
	obs_get_video_info(&ovi);

	switch (autoConfig()->fpsType) {
	case AutoConfig::FPSType::PreferHighFPS:
		autoConfig()->specificFPSNum = 0;
		autoConfig()->specificFPSDen = 0;
		autoConfig()->preferHighFPS = true;
		break;
	case AutoConfig::FPSType::PreferHighRes:
		autoConfig()->specificFPSNum = 0;
		autoConfig()->specificFPSDen = 0;
		autoConfig()->preferHighFPS = false;
		break;
	case AutoConfig::FPSType::UseCurrent:
		autoConfig()->specificFPSNum = ovi.fps_num;
		autoConfig()->specificFPSDen = ovi.fps_den;
		autoConfig()->preferHighFPS = false;
		break;
	case AutoConfig::FPSType::fps30:
		autoConfig()->specificFPSNum = 30;
		autoConfig()->specificFPSDen = 1;
		autoConfig()->preferHighFPS = false;
		break;
	case AutoConfig::FPSType::fps60:
		autoConfig()->specificFPSNum = 60;
		autoConfig()->specificFPSDen = 1;
		autoConfig()->preferHighFPS = false;
		break;
	}
