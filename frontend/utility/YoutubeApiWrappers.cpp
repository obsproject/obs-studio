...
bool YoutubeApiWrappers::GetI18nLanguagesList(QVector<I18nLanguageDescription> &languageListOut)
{
	lastErrorMessage.clear();
	lastErrorReason.clear();
	...
	for (auto &j : json_out["items"].array_items()) {
		languageListOut.push_back(
			{j["id"].string_value().c_str(), j["snippet"]["name"].string_value().c_str()});
	}
	return languageListOut.isEmpty() ? false : true;
}
