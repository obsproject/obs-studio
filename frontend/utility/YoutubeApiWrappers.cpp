...
QVector<I18nLanguageDescription> YoutubeApiWrappers::GetI18nLanguagesList()
{
	lastErrorMessage.clear();
	lastErrorReason.clear();
	QString urlTemplate = QString(youtubeLiveI18nLanguagesUrl.data()) + "?part=snippet&hl=%1";
	QString url = urlTemplate.arg(QLocale().name());

	Json jsonOutput;
	if (!InsertCommand(QT_TO_UTF8(url), "application/json", "", nullptr, jsonOutput)) {
	...
	for (auto &j : jsonOutput["items"].array_items()) {
		languageList.push_back({j["id"].string_value().c_str(), j["snippet"]["name"].string_value().c_str()});
	}
	return languageList;
}
...
bool YoutubeApiWrappers::SetVideoCategory(const QString &video_id, const QString &video_title,
					  const QString &video_description, const QString &category_id,
