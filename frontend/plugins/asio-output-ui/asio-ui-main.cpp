...
namespace {
constexpr int MAX_ASIO_DEVICE_CHANNELS = 32;
constexpr int SHIFT_WINDOW = 100;
...
void save_default_settings(obs_data_t *settings)
{
	...
	for (int i = 0; i < MAX_ASIO_DEVICE_CHANNELS; i++) {
		char key[32];
...
void callback()
{
	...
	if (obsSettingsDialog) {
		...
		QPoint newPos(settingsRect.right() + SHIFT_WINDOW, settingsRect.top());
