...
static void PopulateAdvancedBitrates(std::vector<QPointer<QComboBox>> boxes, const char *stream_id, const char *rec_id)
{
...
void RestrictResetBitrates(std::vector<QPointer<QComboBox>> boxes, int maxbitrate);
...
#undef ADD_HOTKEY_FOCUS_TYPE
	...
	simpleOutRecTracks.reserve(MAX_AUDIO_MIXES);
	advOutStreamSingleTracks.reserve(MAX_AUDIO_MIXES);
	advOutStreamMultiTracks.reserve(MAX_AUDIO_MIXES);
	advOutRecTracks.reserve(MAX_AUDIO_MIXES);
	advOutFlvTracks.reserve(MAX_AUDIO_MIXES);
	advOutFFTracks.reserve(MAX_AUDIO_MIXES);
	advOutputTracksBitrates.reserve(MAX_AUDIO_MIXES);
	advOutputTracksBitrateLabels.reserve(MAX_AUDIO_MIXES);
	advOutputTracksNames.reserve(MAX_AUDIO_MIXES);
	for (size_t i = 0; i < MAX_AUDIO_MIXES; i++) {
		/* SIMPLE MODE */

		auto simpleRecTrack = new QCheckBox(ui->simpleRecTracks);
		simpleRecTrack->setText(QString::number(i + 1));
		simpleRecTrack->setAccessibleName(QTStr("Basic.Settings.Output.Adv.Audio.TrackN").arg(i + 1));
		ui->simpleRecTracks->layout()->addWidget(simpleRecTrack);
		simpleOutRecTracks.emplace_back(simpleRecTrack);
		HookWidget(simpleRecTrack, CHECK_CHANGED, OUTPUTS_CHANGED);

		// Those radio buttons in the Simple mode are decorative
		auto simpleFlvTrack = new QRadioButton(ui->simpleFlvTracks);
		simpleFlvTrack->setText(QString::number(i + 1));
		if (i == 0) {
			simpleFlvTrack->setChecked(true);
		}

		ui->simpleFlvTracks->layout()->addWidget(simpleFlvTrack);

		/* ADVANCED MODE - Stream tab */

		auto advOutStreamSingleTrack = new QRadioButton(ui->streamSingleTracks);
		advOutStreamSingleTrack->setText(QString::number(i + 1));
		advOutStreamSingleTrack->setAccessibleName(QTStr("Basic.Settings.Output.Adv.Audio.TrackN").arg(i + 1));
		ui->streamSingleTracks->layout()->addWidget(advOutStreamSingleTrack);
		advOutStreamSingleTracks.emplace_back(advOutStreamSingleTrack);
		HookWidget(advOutStreamSingleTrack, CHECK_CHANGED, OUTPUTS_CHANGED);

		auto advOutStreamMultiTrack = new QCheckBox(ui->streamMultiTracks);
		advOutStreamMultiTrack->setText(QString::number(i + 1));
		advOutStreamMultiTrack->setAccessibleName(QTStr("Basic.Settings.Output.Adv.Audio.TrackN").arg(i + 1));
		ui->streamMultiTracks->layout()->addWidget(advOutStreamMultiTrack);
		advOutStreamMultiTracks.emplace_back(advOutStreamMultiTrack);
		HookWidget(advOutStreamMultiTrack, CHECK_CHANGED, OUTPUTS_CHANGED);

		/* ADVANCED MODE - Record tab (normal mode) */

		auto advOutRecTrack = new QCheckBox(ui->recTracks);
		advOutRecTrack->setText(QString::number(i + 1));
		advOutRecTrack->setAccessibleName(QTStr("Basic.Settings.Output.Adv.Audio.TrackN").arg(i + 1));
		ui->recTracks->layout()->addWidget(advOutRecTrack);
		advOutRecTracks.emplace_back(advOutRecTrack);
		HookWidget(advOutRecTrack, CHECK_CHANGED, OUTPUTS_CHANGED);
		connect(advOutRecTrack, &QCheckBox::toggled, this, &OBSBasicSettings::AdvReplayBufferChanged);
		connect(advOutRecTrack, &QCheckBox::clicked, this, &OBSBasicSettings::AdvOutRecCheckWarnings);

		auto advOutFlvTrack = new QRadioButton(ui->flvTracks);
		advOutFlvTrack->setText(QString::number(i + 1));
		advOutFlvTrack->setAccessibleName(QTStr("Basic.Settings.Output.Adv.Audio.TrackN").arg(i + 1));
		ui->flvTracks->layout()->addWidget(advOutFlvTrack);
		advOutFlvTracks.emplace_back(advOutFlvTrack);
		HookWidget(advOutFlvTrack, CHECK_CHANGED, OUTPUTS_CHANGED);

		/* ADVANCED MODE - Record tab (FFMpeg mode) */

		auto advOutFFTrack = new QCheckBox(ui->advOutFFTracks);
		advOutFFTrack->setText(QString::number(i + 1));
		advOutFFTrack->setAccessibleName(QTStr("Basic.Settings.Output.Adv.Audio.TrackN").arg(i + 1));
		ui->advOutFFTracks->layout()->addWidget(advOutFFTrack);
		advOutFFTracks.emplace_back(advOutFFTrack);
		HookWidget(advOutFFTrack, CHECK_CHANGED, OUTPUTS_CHANGED);

		/* ADVANCED MODE - Audio tab */

		auto groupBox = new QGroupBox(ui->advOutputAudioTracks);
		groupBox->setTitle(tr("Basic.Settings.Output.Adv.Audio.TrackN").arg(i + 1));
		...
		auto bitrateLabel = new QLabel(groupBox);
		bitrateLabel->setMinimumSize(QSize(170, 0));
		bitrateLabel->setAlignment(Qt::AlignRight | Qt::AlignTrailing | Qt::AlignVCenter);
		bitrateLabel->setText(tr("Basic.Settings.Output.AudioBitrate"));

		auto bitrateInput = new QComboBox(groupBox);
		HookWidget(bitrateInput, COMBO_CHANGED, OUTPUTS_CHANGED);

		formLayout->setWidget(0, QFormLayout::LabelRole, bitrateLabel);
		formLayout->setWidget(0, QFormLayout::FieldRole, bitrateInput);

		auto trackNameLabel = new QLabel(groupBox);
		trackNameLabel->setMinimumSize(QSize(170, 0));
		trackNameLabel->setAlignment(Qt::AlignRight | Qt::AlignTrailing | Qt::AlignVCenter);
		trackNameLabel->setText(tr("Name"));

		auto trackNameInput = new QLineEdit(groupBox);
		HookWidget(trackNameInput, EDIT_CHANGED, OUTPUTS_CHANGED);

		formLayout->setWidget(1, QFormLayout::LabelRole, trackNameLabel);
		formLayout->setWidget(1, QFormLayout::FieldRole, trackNameInput);

		connect(bitrateInput, &QComboBox::currentIndexChanged, this,
			&OBSBasicSettings::UpdateStreamDelayEstimate);
		connect(bitrateInput, &QComboBox::currentIndexChanged, this, &OBSBasicSettings::AdvReplayBufferChanged);

		// There is already a spacer element, so we need to prepend our groups before
		ui->advOutputAudioTracksLayout->insertWidget((int)i, groupBox);

		advOutputTracksBitrates.emplace_back(bitrateInput);
		advOutputTracksBitrateLabels.emplace_back(bitrateLabel);
		advOutputTracksNames.emplace_back(trackNameInput);
	}
...
void OBSBasicSettings::LoadSimpleOutputSettings()
{
	...
	for (size_t i = 0; i < simpleOutRecTracks.size(); ++i) {
		simpleOutRecTracks[i]->setChecked(tracks & (1 << i));
	}
...
void OBSBasicSettings::LoadAdvOutputStreamingSettings()
{
	...
	for (size_t i = 0; i < advOutStreamSingleTracks.size(); i++) {
		if (trackIndex - 1 == (int)i) {
			advOutStreamSingleTracks[i]->setChecked(true);
		}

		advOutStreamMultiTracks[i]->setChecked(audioMixes & (1 << i));
	}
...
void OBSBasicSettings::LoadAdvOutputRecordingSettings()
{
	...
	for (size_t i = 0; i < advOutRecTracks.size(); i++) {
		advOutRecTracks[i]->setChecked(tracks & (1 << i));
	}
	...
	if (flvTrack < (int)advOutFlvTracks.size()) {
		advOutFlvTracks[(size_t)(flvTrack - 1)]->setChecked(true);
	} else {
		advOutFlvTracks[0]->setChecked(true);
	}
}
...
void OBSBasicSettings::LoadAdvOutputFFmpegSettings()
{
	...
	for (size_t i = 0; i < advOutFFTracks.size(); i++) {
		advOutFFTracks[i]->setChecked(audioMixes & (1 << i));
	}
}
...
void OBSBasicSettings::LoadAdvOutputAudioSettings()
{
	...
	PopulateAdvancedBitrates(advOutputTracksBitrates, encoder_id,
				 strcmp(rec_encoder_id, "none") != 0 ? rec_encoder_id : encoder_id);
	...
	// restrict list of bitrates when multichannel is OFF
	if (!IsSurround(speakers)) {
		RestrictResetBitrates(advOutputTracksBitrates, 320);
	}

	for (size_t i = 0; i < advOutputTracksBitrates.size(); i++) {
		std::string trackBitrateConfigKey = "Track" + std::to_string(i + 1) + "Bitrate";
		auto trackBitrateRaw = config_get_uint(main->Config(), "AdvOut", trackBitrateConfigKey.c_str());

		std::string trackNameConfigKey = "Track" + std::to_string(i + 1) + "Name";
		auto trackName = config_get_string(main->Config(), "AdvOut", trackNameConfigKey.c_str());

		auto trackBitrate = FindClosestAvailableAudioBitrate(advOutputTracksBitrates[i], trackBitrateRaw);

		SetComboByName(advOutputTracksBitrates[i], std::to_string(trackBitrate).c_str());
		advOutputTracksNames[i]->setText(trackName);
	}
}
...
static int RadioToIdx(std::vector<QPointer<QRadioButton>> values)
{
	int idx = 1;
...
static int MultiselectToBitmap(std::vector<QPointer<QCheckBox>> values)
{
	int bitmap = 0;
	for (size_t i = 0; i < values.size(); i++) {
		bitmap |= values[i]->isChecked() ? (1 << i) : 0;
	}
...
void OBSBasicSettings::SaveOutputSettings()
{
	...
	config_set_int(main->Config(), "AdvOut", "TrackIndex", RadioToIdx(advOutStreamSingleTracks));
	...
	config_set_int(main->Config(), "AdvOut", "FFAudioMixes", MultiselectToBitmap(advOutFFTracks));

	for (size_t i = 0; i < advOutputTracksBitrates.size(); i++) {
		std::string trackBitrateConfigKey = "Track" + std::to_string(i + 1) + "Bitrate";
		SaveCombo(advOutputTracksBitrates[i], "AdvOut", trackBitrateConfigKey.c_str());

		std::string trackNameConfigKey = "Track" + std::to_string(i + 1) + "Name";
		SaveEdit(advOutputTracksNames[i], "AdvOut", trackNameConfigKey.c_str());
	}

	if (vodTrackCheckbox) {
		SaveCheckBox(simpleVodTrack, "SimpleOutput", "VodTrackEnabled");
		SaveCheckBox(vodTrackCheckbox, "AdvOut", "VodTrackEnabled");
		config_set_int(main->Config(), "AdvOut", "VodTrackIndex", RadioToIdx(vodTracks));
	}
...
void OBSBasicSettings::SpeakerLayoutChanged(int idx)
{
	...
	if (surround) {
		...
		PopulateAdvancedBitrates(advOutputTracksBitrates, stream_encoder_id.c_str(),
...
void OBSBasicSettings::AdvOutRecCheckWarnings()
{
	...
	for (auto track : advOutRecTracks) {
		if (track->isChecked()) {
...
void OBSBasicSettings::UpdateAdvOutStreamDelayEstimate()
{
	...
	int trackIndex = config_get_int(main->Config(), "AdvOut", "TrackIndex");
	auto aBitrateText = advOutputTracksBitrates[trackIndex]->currentText();
...
void OBSBasicSettings::SimpleReplayBufferChanged()
{
	...
	if (ui->simpleOutRecFormat->currentText().compare("flv") == 0 || streamQuality) {
		...
		for (auto track : simpleOutRecTracks) {
			if (track->isChecked()) {
				abitrate += delta;
			}
		}
...
void OBSBasicSettings::AdvReplayBufferChanged()
{
	...
	int abitrate = 0;
	for (auto trackBitrate : advOutputTracksBitrates) {
		abitrate += trackBitrate->currentText().toInt();
	}
...
{
	return RadioToIdx(advOutFlvTracks);
}

int OBSBasicSettings::SimpleOutGetSelectedAudioTracks()
{
	return MultiselectToBitmap(simpleOutRecTracks);
}

int OBSBasicSettings::AdvOutGetSelectedAudioTracks()
{
	return MultiselectToBitmap(advOutRecTracks);
}

int OBSBasicSettings::AdvOutGetStreamingSelectedAudioTracks()
{
	return MultiselectToBitmap(advOutStreamMultiTracks);
}
...
void OBSBasicSettings::UpdateMultitrackVideo()
{
	...
	auto update_advanced_output_audio_tracks = [&](bool mtv_enabled) {
		...
		for (size_t i = 0; i < advOutputTracksBitrates.size(); i++) {
			bool isTrackWarningVisible = mtv_enabled && (advOutStreamSingleTracks[i]->isChecked() ||
								     vod_track_idx_enabled(i + 1));
			bool isTrackDisabled = isTrackWarningVisible && !advOutRecTracks[i]->isChecked();
			advOutputTracksBitrates[i]->setDisabled(isTrackDisabled);
			advOutputTracksBitrateLabels[i]->setDisabled(isTrackDisabled);
		}
	};
...
void OBSBasicSettings::AdvAudioEncodersChanged()
{
	...
	PopulateAdvancedBitrates(advOutputTracksBitrates, QT_TO_UTF8(streamEncoder), QT_TO_UTF8(recEncoder));
