...
void Multiview::Update(MultiviewLayout multiviewLayout, bool drawLabel, bool drawSafeArea)
{
	...
	std::vector<OBSWeakSource> multiviewNewScenes;
	std::vector<OBSSource> multiviewNewLabels;

	multiviewNewLabels.emplace_back(CreateLabel(Str("StudioMode.Preview"), h / 2));
	multiviewNewLabels.emplace_back(CreateLabel(Str("StudioMode.Program"), h / 2));

	size_t i = 0;
	while (i < scenes.sources.num && multiviewNewScenes.size() < maxSrcs) {
		...
		multiviewNewScenes.emplace_back(OBSGetWeakRef(src));
		obs_source_inc_showing(src);

		multiviewNewLabels.emplace_back(CreateLabel(obs_source_get_name(src), h / 3));
	}
	...
	multiviewScenes = multiviewNewScenes;
	multiviewLabels = multiviewNewLabels;
}
