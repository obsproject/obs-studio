	...
	if (item) {
		...
		if (contextBarSize >= ContextBarSize_Reduced && (updateNeeded || force)) {
			...
			if (flags & OBS_SOURCE_CONTROLLABLE_MEDIA) {
				...
				AudioCaptureToolbar *toolbar =
					new AudioCaptureToolbar(ui->contextControlsContainer, source);
				toolbar->Init();
				ui->contextControlsContainer->layout()->addWidget(toolbar);

			} else if (strcmp(id, "wasapi_process_output_capture") == 0) {
				ApplicationAudioCaptureToolbar *toolbar =
				...
				DisplayCaptureToolbar *toolbar =
					new DisplayCaptureToolbar(ui->contextControlsContainer, source);
				toolbar->Init();
				ui->contextControlsContainer->layout()->addWidget(toolbar);

			} else if (strcmp(id, "dshow_input") == 0) {
				DeviceCaptureToolbar *toolbar =
					new DeviceCaptureToolbar(ui->contextControlsContainer, source);
				ui->contextControlsContainer->layout()->addWidget(toolbar);

			} else if (strcmp(id, "game_capture") == 0) {
				GameCaptureToolbar *toolbar =
					new GameCaptureToolbar(ui->contextControlsContainer, source);
				ui->contextControlsContainer->layout()->addWidget(toolbar);

			} else if (strcmp(id, "image_source") == 0) {
				ImageSourceToolbar *toolbar =
					new ImageSourceToolbar(ui->contextControlsContainer, source);
				ui->contextControlsContainer->layout()->addWidget(toolbar);

			} else if (strcmp(id, "color_source") == 0) {
				ColorSourceToolbar *toolbar =
					new ColorSourceToolbar(ui->contextControlsContainer, source);
				ui->contextControlsContainer->layout()->addWidget(toolbar);

			} else if (strcmp(id, "text_ft2_source") == 0 || strcmp(id, "text_gdiplus") == 0) {
				TextSourceToolbar *toolbar =
					new TextSourceToolbar(ui->contextControlsContainer, source);
				ui->contextControlsContainer->layout()->addWidget(toolbar);
			}
		...
		idian::Utils::toggleClass(ui->contextSubContainer, "context-frame", true);
		...
		idian::Utils::toggleClass(ui->contextSubContainer, "context-frame", false);
