...
void OBSBasic::PMAddNewModule(void *param, obs_module_t *newModule)
{
	...
	auto it = std::find_if(obsBasic->pmModules.begin(), obsBasic->pmModules.end(),
			       [&](OBSModuleInfo module) { return module.module_name == moduleName; });

	if (it == obsBasic->pmModules.end()) {
		obsBasic->pmModules.push_back({display_name ? display_name : "", module_name, id ? id : "",
	...
	if (res == QDialog::Accepted) {
		...
		if (changed) {
			QMessageBox::StandardButton button = OBSMessageBox::question(
				this, QTStr("PluginManager.Restart"), QTStr("PluginManager.NeedsRestart"));
