...
QSize OBSSourceWidget::minimumSizeHint()
{
	if (fixedAspectRatio > 0.0) {
		int height = 135;
		int width = int(height * fixedAspectRatio);

		return QSize(width, height);
	}

	return QSize(FALLBACK_WIDTH, FALLBACK_HEIGHT);
}

QSize OBSSourceWidget::sizeHint()
{
	if (sourceView && sourceView->sourceWidth() > 0 && sourceView->sourceHeight() > 0) {
		int width = sourceView->sourceWidth();
		int height = sourceView->sourceHeight();
...
bool OBSSourceWidget::hasHeightForWidth()
{
	return true;
}

int OBSSourceWidget::heightForWidth(int width)
{
