...
static bool move_crop(obs_scene_t * /* scene */, obs_sceneitem_t *item, void *param)
{
	...
	if (selected) {
		...
		/* Move Crop region /
		int xOffset = std::clamp((int)std::floor(offset->x), -crop.left, crop.right);
		int yOffset = std::clamp((int)std::floor(offset->y), -crop.top, crop.bottom);

		crop.left += (int)xOffset;
		crop.right -= (int)xOffset;
		crop.top += (int)yOffset;
		crop.bottom -= (int)yOffset;
		...
		vec2 clampedOffset = {(float)xOffset, (float)yOffset};
		vec2_mul(&clampedOffset, &clampedOffset, &scale);
		move_items(nullptr, item, &clampedOffset);
		/* */
	}
