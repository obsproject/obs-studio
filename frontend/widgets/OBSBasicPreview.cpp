...
static void DrawLine(float x1, float y1, float x2, float y2, float thickness, vec2 scale)
{
	...
	if (!close_float(x1, x2, EPSILON)) {
		thickness /= scale.y;
		cx = x2 - x1;
		cy = thickness;
		thickness_d2 = thickness * 0.5f;
		y1 -= thickness_d2;
	} else {
		thickness /= scale.x;
		cy = y2 - y1;
...
static void DrawStripedLine(float x1, float y1, float x2, float y2, float thickness, vec2 scale)
{
	...
	float dist;
	float dist_scaled;
	bool is_x_axis = !close_float(x1, x2, EPSILON);
	...
	int stripe_count = (int)ceil(dist_scaled / 15);
	if (stripe_count > 100) {
		stripe_count = 100;
	}

	float f_stripes_inv = 1.0f / (float)stripe_count;
	float stripe_len = f_stripes_inv * (dist + thickness);
	...
	for (int i = 0; i < stripe_count; i++) {
		float stripe_start = (float)i * f_stripes_inv;
		float x = is_x_axis ? stripe_start : x1;
		float y = is_x_axis ? y1 : stripe_start;
