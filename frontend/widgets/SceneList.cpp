...
void SceneList::createContextMenuForItem(QListWidgetItem *item)
{
	...
	OBSSourceAutoRelease tmp = obs_get_source_by_uuid(uuid.toUtf8().constData());
	OBSSource source = tmp.Get();
	...
	connect(copyFilters, &QAction::triggered, main, [main, source]() { main->copyFiltersFromSource(source); });
	auto *pasteFilters = new QAction(QTStr("Paste.Filters"), this);
	pasteFilters->setEnabled(main->canPasteFilters());
	connect(pasteFilters, &QAction::triggered, main, [main, source]() { main->pasteFiltersToSource(source); });
	...
	for (int i = 0; i < projectors.size(); i++) {
		...
		QAction *action = sceneProjectorMenu->addAction(actionName, main, [main, source]() {
			main->openProjectorForSource(source);
		});
		action->setProperty("monitor", i);
	}
	sceneProjectorMenu->addSeparator();
	sceneProjectorMenu->addAction(QTStr("Projector.Window"), main,
				      [main, source]() { main->openWindowedProjectorForSource(source); });

	contextMenu->addMenu(sceneProjectorMenu);
	contextMenu->addSeparator();

	contextMenu->addAction(QTStr("Screenshot.Scene"), main, [main, scene]() { main->screenshotScene(scene); });
	contextMenu->addSeparator();
...
void SceneList::applySelectionFromMediator(const OBSWeakSource &weak)
{
...
void SceneList::onSceneRenamed(const OBSWeakSource &weak)
{
...
void SceneList::attachMediator(OBS::CanvasMediator *mediator)
{
	...
	connections.push_back(
		connect(this, &SceneList::sceneSelected, safeMediator, [safeMediator](const OBSWeakSource &weak) {
			OBSSource source = OBSGetStrongRef(weak);
			OBSScene scene = obs_scene_from_source(source);
