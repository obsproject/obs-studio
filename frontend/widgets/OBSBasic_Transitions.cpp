...
void OBSBasic::RenameTransition(OBSSource transition)
{
	...
	transitionNameToUuids.insert({name, obs_source_get_uuid(source)});

	emit TransitionRenamed(QT_UTF8(obs_source_get_uuid(source)), QString::fromStdString(name));
...
obs_data_array_t *OBSBasic::SaveTransitions()
{
	...
	for (const auto &[uuid, transition] : transitions) {
		...
		obs_data_set_string(sourceData, "name", obs_source_get_name(transition));
		obs_data_set_string(sourceData, "id", obs_obj_get_id(transition));
