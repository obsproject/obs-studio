...
{
	int rule_power = 0;
	if (rule->mask & WINDOW_MATCH_EXE)
		rule_power += 2;
	if (rule->mask & WINDOW_MATCH_CLASS)
		rule_power += 2;
	if (rule->mask & WINDOW_MATCH_TITLE)
		rule_power += 2;

	if (rule->type == WINDOW_MATCH_CAPTURE)
		rule_power += 1;
	if (rule->type == WINDOW_MATCH_IGNORE)
		rule_power += 2;
