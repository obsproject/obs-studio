...
bool cf_preprocess_get_def_value(struct cf_preprocessor *pp,
				 const struct strref *def_name, int *value)
{
	...
	for (size_t i = 0; i < def->tokens.num; i++) {
		...
		if (def->tokens.array[i].type == CFTOKEN_NAME &&
		    cf_preprocess_get_def_value(pp, &def->tokens.array[i].str,
						value))
			return true;
...
static void cf_preprocess_if(struct cf_preprocessor *pp,
			     struct cf_token **p_cur_token)
{
	struct cf_token *cur_token = *p_cur_token;
	...
	next_token(&cur_token, true);
	if ((cur_token->type != CFTOKEN_NAME ||
	     !cf_preprocess_get_def_value(pp, &cur_token->str, &value_a)) &&
	    (cur_token->type != CFTOKEN_NUM ||
	     sscanf(cur_token->str.array, "%i", &value_a) != 1)) {
	...
	if (cur_token->type == CFTOKEN_NEWLINE) {
		...
		next_token(&cur_token, true);
		if ((cur_token->type != CFTOKEN_NAME ||
		     !cf_preprocess_get_def_value(pp, &cur_token->str,
						  &value_b)) &&
		    (cur_token->type != CFTOKEN_NUM ||
		     sscanf(cur_token->str.array, "%i", &value_b) != 1)) {
