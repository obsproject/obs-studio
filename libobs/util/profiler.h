...
EXPORT void profile_register_root(
		const char *name, uint64_t expected_time_between_calls);
...
PRINTFATTR(2, 3)
EXPORT const char *profile_store_name(
		profiler_name_store_t *store, const char *format, ...);
...
typedef bool (*profiler_entry_enum_func)(
		void *context, profiler_snapshot_entry_t *entry);

EXPORT profiler_snapshot_t *profile_snapshot_create(void);
EXPORT void                 profile_snapshot_free(profiler_snapshot_t *snap);

EXPORT bool profiler_snapshot_dump_csv(
		const profiler_snapshot_t *snap, const char *filename);
EXPORT bool profiler_snapshot_dump_csv_gz(
		const profiler_snapshot_t *snap, const char *filename);
...
typedef bool (*profiler_name_filter_func)(
		void *data, const char *name, bool *remove);
