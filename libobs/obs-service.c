...
obs_properties_t *obs_get_service_properties(const char *id, obs_data_t *settings)
{
	const struct obs_service_info *info = find_service(id);
	if (info && info->get_properties) {
		obs_properties_t *properties;

		properties = info->get_properties(NULL, settings);
...
obs_properties_t *obs_service_properties(const obs_service_t *service)
{
	...
	if (service->info.get_properties) {
		...
		props = service->info.get_properties(service->context.data, service->context.settings);
