...
EXPORT void bounds_get_point(
		struct vec3 *dst, const struct bounds *b, unsigned int i);
...
EXPORT bool bounds_intersects_obb(const struct bounds *b,
		const struct bounds *test, const struct matrix4 *m, float epsilon);
EXPORT bool bounds_intersects_obb3x4(const struct bounds *b,
		const struct bounds *test, const struct matrix3 *m, float epsilon);
