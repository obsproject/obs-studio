...
struct gs_exports {
	...
	bool (*device_enum_adapters)(
			bool (*callback)(void *, const char *, uint32_t), void *);
	...
	void (*device_get_size)(
			const gs_device_t *device, uint32_t *x, uint32_t *y);
	...
	void (*device_load_vertexbuffer)(
			gs_device_t *device, gs_vertbuffer_t *vertbuffer);
	void (*device_load_indexbuffer)(
			gs_device_t *device, gs_indexbuffer_t *indexbuffer);
	void (*device_load_texture)(
			gs_device_t *device, gs_texture_t *tex, int unit);
	void (*device_load_samplerstate)(gs_device_t *device,
			gs_samplerstate_t *samplerstate, int unit);
	void (*device_load_vertexshader)(
			gs_device_t *device, gs_shader_t *vertshader);
	void (*device_load_pixelshader)(
			gs_device_t *device, gs_shader_t *pixelshader);
	void (*device_load_default_samplerstate)(
			gs_device_t *device, bool b_3d, int unit);
	...
	void (*device_load_swapchain)(
			gs_device_t *device, gs_swapchain_t *swaphchain);
	...
	void (*device_get_viewport)(
			const gs_device_t *device, struct gs_rect *rect);
	void (*device_set_scissor_rect)(
			gs_device_t *device, const struct gs_rect *rect);
	...
	bool (*gs_texture_map)(
			gs_texture_t *tex, uint8_t **ptr, uint32_t *linesize);
	...
	gs_sparam_t *(*gs_shader_get_param_by_idx)(
			gs_shader_t *shader, uint32_t param);
	gs_sparam_t *(*gs_shader_get_param_by_name)(
	...
	void (*gs_shader_set_matrix3)(
			gs_sparam_t *param, const struct matrix3 *val);
	void (*gs_shader_set_matrix4)(
			gs_sparam_t *param, const struct matrix4 *val);
	...
	gs_duplicator_t *(*device_duplicator_create)(
	...
	gs_texture_t *(*device_texture_create_gdi)(
			gs_device_t *device, uint32_t width, uint32_t height);

	void *(*gs_texture_get_dc)(gs_texture_t *gdi_tex);
	void (*gs_texture_release_dc)(gs_texture_t *gdi_tex);

	gs_texture_t *(*device_texture_open_shared)(
			gs_device_t *device, uint32_t handle);
	uint32_t (*device_texture_get_shared_handle)(gs_texture_t *tex);
	int (*device_texture_acquire_sync)(
			gs_texture_t *tex, uint64_t key, uint32_t ms);
	...
	gs_stagesurf_t *(*device_stagesurface_create_nv12)(
			gs_device_t *device, uint32_t width, uint32_t height);
#endif
};
