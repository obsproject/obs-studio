...
EXPORT gs_swapchain_t *device_swapchain_create(
...
EXPORT gs_texture_t *device_voltexture_create(gs_device_t *device, uint32_t width,
		uint32_t height, uint32_t depth, enum gs_color_format color_format,
		uint32_t levels, const uint8_t **data, uint32_t flags);
...
EXPORT gs_samplerstate_t *device_samplerstate_create(
...
EXPORT gs_shader_t *device_pixelshader_create(gs_device_t *device,
		const char *shader, const char *file, char **error_string);
EXPORT gs_vertbuffer_t *device_vertexbuffer_create(
		gs_device_t *device, struct gs_vb_data *data, uint32_t flags);
...
EXPORT void                 device_load_vertexbuffer(
				gs_device_t *device, gs_vertbuffer_t *vertbuffer);
EXPORT void device_load_indexbuffer(
...
EXPORT void device_load_samplerstate(
		gs_device_t *device, gs_samplerstate_t *samplerstate, int unit);
...
EXPORT void device_load_default_samplerstate(
...
EXPORT void device_copy_texture(
		gs_device_t *device, gs_texture_t *dst, gs_texture_t *src);
EXPORT void device_copy_texture_region(gs_device_t *device, gs_texture_t *dst,
		uint32_t dst_x, uint32_t dst_y, gs_texture_t *src,
		uint32_t src_x, uint32_t src_y, uint32_t src_w, uint32_t src_h);
EXPORT void device_stage_texture(
		gs_device_t *device, gs_stagesurf_t *dst, gs_texture_t *src);
...
EXPORT void device_set_viewport(
		gs_device_t *device, int x, int y, int width, int height);
EXPORT void device_get_viewport(const gs_device_t *device, struct gs_rect *rect);
EXPORT void device_set_scissor_rect(
