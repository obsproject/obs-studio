...
static void init_hold_hotkeys(obs_scene_t *scene, obs_sceneitem_t *item, const char *name);
...
static void hotkey_show_hold(void *data, obs_hotkey_id id, obs_hotkey_t *hotkey, bool pressed)
{
	UNUSED_PARAMETER(id);
	UNUSED_PARAMETER(hotkey);
...
static void hotkey_hide_hold(void *data, obs_hotkey_id id, obs_hotkey_t *hotkey, bool pressed)
{
	UNUSED_PARAMETER(id);
	UNUSED_PARAMETER(hotkey);
...
static void init_hold_hotkeys(obs_scene_t *scene, obs_sceneitem_t *item, const char *name)
{
	...
	item->show_hold = obs_hotkey_register_source(scene->source, show_hold.array, show_hold_desc.array,
						     hotkey_show_hold, item);

	item->hide_hold = obs_hotkey_register_source(scene->source, hide_hold.array, hide_hold_desc.array,
...
static void sceneitem_rename_hold_hotkey(const obs_sceneitem_t *scene_item, const char *new_name)
{
	struct dstr show_hold = {0};
	struct dstr hide_hold = {0};
	struct dstr show_hold_desc = {0};
