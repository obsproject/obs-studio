...
static bool obs_init_audio_mix(struct audio_output_info *ai, struct obs_core_audio_mix *audio)
{
	audio->ai = *ai;
...
struct obs_core_audio_mix *obs_create_audio_mix(struct audio_output_info *ai)
{
	struct obs_core_audio_mix *audio = bzalloc(sizeof(struct obs_core_audio_mix));
	if (obs_init_audio_mix(ai, audio) != AUDIO_OUTPUT_SUCCESS) {
		bfree(audio);
		audio = NULL;
	}
	return audio;
...
static bool obs_init_audio(const struct obs_audio_info2 *oai)
{
	...
	/* Reset main canvas mix first so it remains first in the rendering order. */
	if (!obs_canvas_reset_audio_internal(obs->data.main_canvas, &ai))
		return false;
...
static void stop_audio(void)
{
	...
	for (size_t i = 0, num = obs->audio.mixes.num; i < num; i++) {
		audio_output_close(obs->audio.mixes.array[i]->audio);
		obs->audio.mixes.array[i]->audio = NULL;
	}
