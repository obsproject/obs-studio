...
static void maybe_set_up_gpu_rescale(struct obs_encoder *encoder)
{
	...
	/* If a preferred format is not already set, allow the encoder to tell use which format it prefers to determine
	 * if we should opportunistically create a conversion mix even if scaling is disabled. */
