...
static obs_canvas_t *obs_canvas_create_internal(const char *name, const char *uuid, struct obs_video_info *ovi,
						struct audio_output_info *ai, uint32_t flags, bool private)
{
	...
	if (ai) {
		canvas->ai = *ai;
		canvas->audio_mix = obs_create_audio_mix(ai);
		if (canvas->audio_mix) {
...
obs_canvas_t *obs_canvas_create2(const char *name, struct obs_video_info *ovi, struct audio_output_info *ai,
				 uint32_t flags)
{
	flags &= ~MAIN; /* Prevent user from creating a MAIN canvas. */
	return obs_canvas_create_internal(name, NULL, ovi, ai, flags, false);
}

obs_canvas_t *obs_canvas_create_private2(const char *name, struct obs_video_info *ovi, struct audio_output_info *ai,
					 uint32_t flags)
{
	flags &= ~MAIN; /* Prevent user from creating a MAIN canvas. */
	return obs_canvas_create_internal(name, NULL, ovi, ai, flags, true);
}
...
bool obs_canvas_reset_audio_internal(obs_canvas_t *canvas, struct audio_output_info *ai)
{
	if (!ai && !canvas->audio_mix)
		return true;
	...
	canvas->audio_mix = obs_create_audio_mix(&canvas->ai);
	if (canvas->audio_mix) {
		pthread_mutex_lock(&obs->audio.mixes_mutex);
		da_push_back(obs->audio.mixes, &canvas->audio_mix);
		pthread_mutex_unlock(&obs->audio.mixes_mutex);
...
bool obs_canvas_reset_audio(obs_canvas_t *canvas, struct audio_output_info *ai)
{
	if (canvas->flags & MAIN || obs_audio_active())
		return false;

	return obs_canvas_reset_audio_internal(canvas, ai);
}
...
bool obs_canvas_get_audio_info(const obs_canvas_t *canvas, struct audio_output_info *ai)
{
	if (!canvas->audio_mix)
		return false;
