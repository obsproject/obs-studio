...
bool obs_x11_hotkeys_platform_init(struct obs_core_hotkeys *hotkeys)
{
...
void obs_x11_hotkeys_platform_free(struct obs_core_hotkeys *hotkeys)
{
	obs_hotkeys_platform_t *context = hotkeys->platform_context;
...
bool obs_x11_hotkeys_platform_is_pressed(obs_hotkeys_platform_t *context,
					 obs_key_t key)
{
...
void obs_x11_key_to_str(obs_key_t key, struct dstr *dstr)
{
	if (key >= OBS_KEY_MOUSE1 && key <= OBS_KEY_MOUSE29) {
...
obs_key_t obs_x11_key_from_virtual_key(int sym)
{
	obs_hotkeys_platform_t *context = obs->hotkeys.platform_context;
...
int obs_x11_key_to_virtual_key(obs_key_t key)
{
	if (key == OBS_KEY_META)
		return XK_Super_L;
