...
static void gl_nix_egl_wayland_windowinfo_destroy(struct gl_windowinfo *info)
{
...
static void egl_bind_opengl(void)
{
...
static struct gl_platform *
gl_nix_egl_wayland_platform_create(gs_device_t *device, uint32_t adapter)
{
	struct gl_platform *plat = bmalloc(sizeof(struct gl_platform));

	plat->wl_display = obs_get_platform_display();
...
static void gl_nix_egl_wayland_platform_destroy(struct gl_platform *plat)
{
	if (plat) {
		egl_context_destroy(plat);
		eglTerminate(plat->display);
...
static bool
gl_nix_egl_wayland_platform_init_swapchain(struct gs_swap_chain *swap)
{
	struct gl_platform *plat = swap->device->plat;
	EGLSurface surface = eglCreateWindowSurface(plat->display, plat->config,
						    swap->wi->window, NULL);
...
static void
gl_nix_egl_wayland_platform_cleanup_swapchain(struct gs_swap_chain *swap)
{
	struct gl_platform *plat = swap->device->plat;
	eglDestroySurface(plat->display, swap->wi->surface);
}

static void gl_nix_egl_wayland_device_enter_context(gs_device_t *device)
{
	struct gl_platform *plat = device->plat;
	EGLSurface surface = EGL_NO_SURFACE;
	if (device->cur_swap != NULL)
		surface = device->cur_swap->wi->surface;
	egl_make_current(plat->display, surface, plat->context);
	egl_bind_opengl();
}

static void gl_nix_egl_wayland_device_leave_context(gs_device_t *device)
{
	struct gl_platform *plat = device->plat;
	egl_make_current(plat->display, EGL_NO_SURFACE, EGL_NO_CONTEXT);
}

static void *gl_nix_egl_wayland_device_get_device_obj(gs_device_t *device)
{
	return device->plat->context;
}

static void gl_nix_egl_wayland_getclientsize(const struct gs_swap_chain *swap,
...
static void gl_nix_egl_wayland_clear_context(gs_device_t *device)
{
	struct gl_platform *plat = device->plat;
	egl_make_current(plat->display, EGL_NO_SURFACE, EGL_NO_CONTEXT);
}

static void gl_nix_egl_wayland_update(gs_device_t *device)
{
...
static void gl_nix_egl_wayland_device_load_swapchain(gs_device_t *device,
						     gs_swapchain_t *swap)
{
	...
	if (swap == NULL) {
		...
		egl_make_current(plat->display, swap->wi->surface,
				 plat->context);
	}
}

static void gl_nix_egl_wayland_device_present(gs_device_t *device)
{
	struct gl_platform *plat = device->plat;
	...
	if (eglSwapBuffers(plat->display, wi->surface) == EGL_FALSE) {
		blog(LOG_ERROR, "eglSwapBuffers failed");
	}
}

static const struct gl_winsys_vtable egl_wayland_winsys_vtable = {
	.windowinfo_create = gl_nix_egl_wayland_windowinfo_create,
	.windowinfo_destroy = gl_nix_egl_wayland_windowinfo_destroy,
	.platform_create = gl_nix_egl_wayland_platform_create,
	.platform_destroy = gl_nix_egl_wayland_platform_destroy,
	.platform_init_swapchain = gl_nix_egl_wayland_platform_init_swapchain,
	.platform_cleanup_swapchain =
		gl_nix_egl_wayland_platform_cleanup_swapchain,
	.device_enter_context = gl_nix_egl_wayland_device_enter_context,
	.device_leave_context = gl_nix_egl_wayland_device_leave_context,
	.device_get_device_obj = gl_nix_egl_wayland_device_get_device_obj,
	.getclientsize = gl_nix_egl_wayland_getclientsize,
	.clear_context = gl_nix_egl_wayland_clear_context,
	.update = gl_nix_egl_wayland_update,
	.device_load_swapchain = gl_nix_egl_wayland_device_load_swapchain,
	.device_present = gl_nix_egl_wayland_device_present,
};

const struct gl_winsys_vtable *gl_nix_egl_wayland_get_winsys_vtable(void)
{
